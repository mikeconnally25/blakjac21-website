import {
  buildAuthorizeUrl,
  createPkcePair,
  exchangeCodeForToken,
  fetchKickUser,
} from "../lib/kick-auth.js";
import { ensureKickConfig, getKickConfig } from "../lib/config.js";
import { isKickAdmin } from "../lib/admins.js";
import {
  appendSetCookies,
  buildSessionCookie,
  clearCookie,
  clearSession,
  consumeOAuthState,
  createOAuthCookie,
  createSignedOAuthState,
  getBotOAuthState,
  getOAuthState,
  getRawSession,
} from "../lib/session.js";

import { ensureUserRecord, getUserByKickId, setUserBanned, upsertKickUser, withLiveUserBadges } from "../lib/users.js";
import { getClientGeo, getClientIp, getUserAgent } from "../lib/client-ip.js";
import { handleKickBotCallback } from "./kick-bot-auth-handlers.js";

function buildBonusHuntBotRedirect(params) {
  const url = new URL("/bonus-hunt/", "http://local");
  Object.entries(params).forEach(([key, value]) => {
    url.searchParams.set(key, value);
  });
  return `${url.pathname}${url.search}`;
}

function redirect(res, location, cookies) {
  if (cookies) {
    appendSetCookies(res, cookies);
  }
  res.statusCode = 302;
  res.setHeader("Location", location);
  res.end();
}

function authErrorMessage({ code, state, oauth }) {
  if (!code) {
    return "Kick did not return an authorization code. Check that your redirect URI matches exactly in Kick developer settings.";
  }
  if (!state) {
    return "Kick did not return OAuth state. Try signing in again.";
  }
  if (!oauth) {
    return "Sign-in session expired. Close extra Kick tabs and try signing in again.";
  }
  return "Missing authorization data";
}

function sanitizeReturnTo(value) {
  if (!value || typeof value !== "string") return "/";
  if (!value.startsWith("/") || value.startsWith("//")) return "/";
  return value;
}

function buildAuthRedirect(returnTo, params) {
  const path = sanitizeReturnTo(returnTo);
  const url = new URL(path, "http://local");
  Object.entries(params).forEach(([key, val]) => {
    url.searchParams.set(key, val);
  });
  return `${url.pathname}${url.search}`;
}

export async function handleLogin(req, res) {
  const config = getKickConfig(req);
  ensureKickConfig(config);

  const loginUrl = new URL(req.url, config.baseUrl);
  const returnTo = sanitizeReturnTo(loginUrl.searchParams.get("returnTo"));

  const { codeVerifier, codeChallenge } = createPkcePair();
  // Signed state carries PKCE verifier so callbacks work without cookies/Redis.
  const state = await createSignedOAuthState({
    codeVerifier,
    returnTo,
    kind: "user",
  });

  const oauthCookie = await createOAuthCookie({ state, codeVerifier, returnTo });

  const authorizeUrl = buildAuthorizeUrl({
    clientId: config.clientId,
    redirectUri: config.redirectUri,
    scopes: ["user:read"],
    state,
    codeChallenge,
  });

  redirect(res, authorizeUrl, [oauthCookie, clearCookie("kick_bot_oauth")]);
}

export async function handleCallback(req, res) {
  const config = getKickConfig(req);
  ensureKickConfig(config);

  const url = new URL(req.url, config.baseUrl);
  const code = url.searchParams.get("code");
  const state = url.searchParams.get("state");
  const error = url.searchParams.get("error");
  const errorDescription = url.searchParams.get("error_description");
  const oauth = await getOAuthState(req, state);
  const botOAuth = await getBotOAuthState(req, state);

  const returnTo = sanitizeReturnTo(oauth?.returnTo);

  if (error) {
    if (botOAuth) {
      redirect(
        res,
        buildBonusHuntBotRedirect({
          kickBot: "error",
          message: errorDescription || error,
        }),
        clearCookie("kick_bot_oauth")
      );
      return;
    }

    redirect(
      res,
      buildAuthRedirect(returnTo, {
        auth: "error",
        message: errorDescription || error,
      })
    );
    return;
  }

  if (botOAuth && state === botOAuth.state) {
    return handleKickBotCallback(req, res, { code, state, oauth: botOAuth });
  }

  if (!code || !state || !oauth) {
    redirect(
      res,
      buildAuthRedirect(returnTo, {
        auth: "error",
        message: authErrorMessage({ code, state, oauth }),
      })
    );
    return;
  }

  if (state !== oauth.state) {
    redirect(
      res,
      buildAuthRedirect(returnTo, {
        auth: "error",
        message: "Invalid OAuth state",
      })
    );
    return;
  }

  try {
    const tokens = await exchangeCodeForToken({
      code,
      clientId: config.clientId,
      clientSecret: config.clientSecret,
      redirectUri: config.redirectUri,
      codeVerifier: oauth.codeVerifier,
    });

    const kickUser = await fetchKickUser(tokens.access_token);
    const kickUserId = String(
      kickUser?.user_id ?? kickUser?.id ?? kickUser?.userId ?? ""
    );
    if (kickUserId) {
      const existing = await getUserByKickId(kickUserId);
      if (existing?.banned && !isKickAdmin(kickUser)) {
        const altMessage =
          String(existing.banReason || "") === "alt"
            ? "This account was permanently banned as an alt. It cannot register or sign in again."
            : "This account has been banned.";
        await consumeOAuthState(state);
        redirect(
          res,
          buildAuthRedirect(oauth.returnTo, {
            auth: "error",
            message: altMessage,
          }),
          clearCookie("kick_oauth")
        );
        return;
      }
      if (existing?.banned && isKickAdmin(kickUser)) {
        await setUserBanned(kickUserId, false);
      }
    }

    const geo = getClientGeo(req);
    const user = await upsertKickUser(kickUser, {
      ip: getClientIp(req),
      userAgent: getUserAgent(req),
      city: geo?.city || null,
      region: geo?.region || null,
      country: geo?.country || null,
    });

    if (user?.banned && !isKickAdmin(kickUser)) {
      const altMessage =
        String(user.banReason || "") === "alt"
          ? "This account was permanently banned as an alt. It cannot register or sign in again."
          : "This account has been banned.";
      await consumeOAuthState(state);
      redirect(
        res,
        buildAuthRedirect(oauth.returnTo, {
          auth: "error",
          message: altMessage,
        }),
        clearCookie("kick_oauth")
      );
      return;
    }

    if (user?.banned && isKickAdmin(kickUser)) {
      await setUserBanned(user.kickUserId, false);
    }

    const sessionCookie = await buildSessionCookie({
      kickUserId: user.kickUserId,
      username: user.username,
      profilePicture: user.profilePicture,
      isNew: user.isNew,
      isAdmin: isKickAdmin(kickUser),
    });

    const redirectFlag = user.isNew ? "created" : "signed-in";
    await consumeOAuthState(state);
    redirect(res, buildAuthRedirect(oauth.returnTo, { auth: redirectFlag }), [
      sessionCookie,
      clearCookie("kick_oauth"),
    ]);
  } catch (err) {
    console.error("OAuth callback failed:", err);
    redirect(
      res,
      buildAuthRedirect(oauth.returnTo, {
        auth: "error",
        message: err.message,
      })
    );
  }
}

export async function handleMe(req, res) {
  const session = await getRawSession(req);

  if (!session) {
    res.statusCode = 401;
    res.setHeader("Content-Type", "application/json");
    res.end(JSON.stringify({ authenticated: false }));
    return;
  }

  const storedUser = await getUserByKickId(session.kickUserId);
  const sessionIsAdmin =
    Boolean(session.isAdmin) ||
    isKickAdmin({
      name: session.username,
      username: session.username,
      user_id: session.kickUserId,
      kickUserId: session.kickUserId,
    });

  if (storedUser?.banned && sessionIsAdmin) {
    await setUserBanned(session.kickUserId, false);
  } else if (storedUser?.banned) {
    clearSession(res);
    res.statusCode = 401;
    res.setHeader("Content-Type", "application/json");
    res.end(
      JSON.stringify({
        authenticated: false,
        error: "This account has been banned.",
      })
    );
    return;
  }

  const { isParticipationBlocked } = await import("./users.js");
  if (await isParticipationBlocked(session.kickUserId)) {
    clearSession(res);
    res.statusCode = 401;
    res.setHeader("Content-Type", "application/json");
    res.end(
      JSON.stringify({
        authenticated: false,
        error: "This account is blocked and cannot participate.",
      })
    );
    return;
  }

  try {
    await ensureUserRecord({
      kickUserId: session.kickUserId,
      username: session.username,
      profilePicture: session.profilePicture,
    });
  } catch (err) {
    console.error("Could not persist user record:", err);
  }

  const refreshedUser = (await getUserByKickId(session.kickUserId)) || storedUser;
  const user = await withLiveUserBadges({
    ...session,
    stakeUsername: refreshedUser?.stakeUsername ?? null,
    stakeLinkedAt: refreshedUser?.stakeLinkedAt ?? null,
    stakeCodeVerified: refreshedUser?.stakeCodeVerified ?? false,
    stakeCodeVerifiedAt: refreshedUser?.stakeCodeVerifiedAt ?? null,
    affGranted: Boolean(refreshedUser?.affGranted),
  });

  // Sliding session: extend cookie so active users stay signed in.
  try {
    const sessionCookie = await buildSessionCookie({
      kickUserId: session.kickUserId,
      username: user.username || session.username,
      profilePicture: user.profilePicture || session.profilePicture,
      isNew: Boolean(session.isNew),
      isAdmin: sessionIsAdmin,
    });
    appendSetCookies(res, sessionCookie);
  } catch (err) {
    console.error("Could not refresh session cookie:", err.message);
  }

  res.statusCode = 200;
  res.setHeader("Content-Type", "application/json");
  res.end(
    JSON.stringify({
      authenticated: true,
      user,
    })
  );
}

export async function handleLogout(req, res) {
  clearSession(res);
  res.statusCode = 200;
  res.setHeader("Content-Type", "application/json");
  res.end(JSON.stringify({ ok: true }));
}
