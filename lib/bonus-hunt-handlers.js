...
function enrichSlotRequests(requests, catalog) {
  return requests.map((request) => {
    ...
    return {
      ...
      thumbnailUrl: request.thumbnailUrl || slot?.thumbnailUrl || null,
    };
  });
}
...
export async function handleBonusHuntRequestsList(req, res) {
  try {
    ...
    const payload = {
      requests: enrichSlotRequests(requests, catalog),
...
export async function handleBonusHuntRequestSubmit(req, res) {
  ...
  try {
    ...
    const [enriched] = enrichSlotRequests([request], catalog);

    sendJson(res, 200, { ok: true, request: enriched });
  } catch (error) {
    sendJson(res, 400, { error: error.message });
  }
}
