...
import {
  assertStakeSyncToken,
  completeStakeSyncToken,
  createStakeSyncToken,
  getStakeSyncStatus,
  updateStakeSyncProgress,
} from "./stake-sync-token.js";
import { cleanEnv } from "./config.js";

let lastAutoSlotRefreshAt = 0;
const SLOT_CATALOG_STALE_MS = 15 * 60 * 1000;
const SLOT_AUTO_REFRESH_COOLDOWN_MS = 60 * 1000;

function isSlotCatalogStale(catalog) {
  ...
  return Date.now() - updatedAt > SLOT_CATALOG_STALE_MS;
}
...
export async function handleBonusHuntSlots(req, res) {
  try {
    ...
    const shouldAutoRefresh =
      isStakeSyncConfigured() &&
      isSlotCatalogStale(catalog) &&
      Date.now() - lastAutoSlotRefreshAt > SLOT_AUTO_REFRESH_COOLDOWN_MS;

    if (shouldAutoRefresh) {
      lastAutoSlotRefreshAt = Date.now();
      try {
        const result = await refreshAllowedSlotCatalogIncremental({ pages: 2 });
        catalog = result.catalog;
      } catch {
        // Keep the last known catalog state; health is recorded by refresh.
      }
    }

    const sectionStats = getCatalogSectionCounts(catalog);
    const health = await getStakeSyncHealth();
    ...
    sendJson(res, 200, {
      ...
      slots: catalog.slots,
      sections: sectionStats.sections,
      unique: sectionStats.unique,
...
function assertCronAuth(req, res) {
  ...
  sendJson(res, 401, {
    error: "Unauthorized. Use Authorization: Bearer $CRON_SECRET or Vercel Cron.",
  });
  return false;
}

export async function handleBonusHuntSlotsAutoSync(req, res) {
  ...
  if (!isStakeSyncConfigured()) {
    sendJson(res, 503, {
      ...
      syncError: "STAKE_ACCESS_TOKEN is not configured.",
      error: "STAKE_ACCESS_TOKEN is not configured.",
    });
    return;
  }

  try {
    // Keep ticks small so this fits under the shared api maxDuration (10s).
    const result = await refreshAllowedSlotCatalogIncremental({ pages: 2 });
    const health = await getStakeSyncHealth();
    sendJson(res, 200, {
      ok: true,
      updatedAt: result.catalog.updatedAt,
      count: result.catalog.slots.length,
      ...
      syncOk: health.syncOk,
      cookieConfigured: health.cookieConfigured,
      ...getSlotCatalogThumbnailStats(result.catalog.slots),
    });
  } catch (error) {
    const health = await getStakeSyncHealth();
    sendJson(res, 502, {
      ok: false,
      syncConfigured: health.syncConfigured,
      syncError: health.syncError || formatStakeCatalogError(error),
      syncOk: false,
      cookieConfigured: health.cookieConfigured,
...
export async function handleBonusHuntSlotsRefresh(req, res) {
  ...
  try {
    const result = await refreshAllowedSlotCatalogIncremental();
    const health = await getStakeSyncHealth();
    sendJson(res, 200, {
      ok: true,
      updatedAt: result.catalog.updatedAt,
...
export async function handleBonusHuntSlotsImportSync(req, res) {
  ...
  try {
    await assertStakeSyncToken(token);

    if (isDone && !body.payload && !body.graphql && !body.imports && !body.slots) {
      const catalog = await persistSlotCatalog(await getAllowedSlotCatalog());
      const stats = getSlotCatalogThumbnailStats(catalog.slots);
      const sectionStats = getCatalogSectionCounts(catalog);
      await completeStakeSyncToken(token, {
        count: sectionStats.unique || catalog.slots.length,
        withThumbnails: stats.withThumbnails,
      });
      sendJson(res, 200, {
        ok: true,
        done: true,
    ...
    const payload = body.payload ?? body.graphql ?? body.imports ?? body;
    const catalog = await importAllowedSlotCatalog(payload, {
      light: Boolean(isPartial && !isDone),
    });
    const stats = getSlotCatalogThumbnailStats(catalog.slots);
    const sectionStats = getCatalogSectionCounts(catalog);
    ...
    if (isPartial && !isDone) {
      await updateStakeSyncProgress(token, {
        count: sectionStats.unique || catalog.slots.length,
        withThumbnails: stats.withThumbnails,
        progress,
      });
