...
function isStakeSyncOrigin(origin) {
  try {
...
function setStakeSyncCors(req, res) {
  ...
  if (origin && isStakeSyncOrigin(origin)) {
...
export async function handleBonusHuntSlotsSyncStatus(req, res) {
  ...
  const token = String(getRequestQuery(req).token || "").trim();
  if (!token) {
...
export async function handleBonusHuntSlotsImportSync(req, res) {
  ...
  try {
    ...
    const payload = body.payload ?? body.graphql ?? body.imports ?? body;
    // importAllowedSlotCatalog already persists the merged catalog.
    const catalog = await importAllowedSlotCatalog(payload);
    ...
    sendJson(res, 400, { error: error.message || "Could not import slots." });
  }
}
...
async function enrichBonuses(bonuses, catalog) {
  return Promise.all(
    bonuses.map(async (bonus) => {
      ...
      const provider = bonus.provider || slot?.provider || null;
      const cleanName =
        stripProviderFromSlotName(slot?.name || bonus.slot, provider) ||
        bonus.slot;
    ...
    if (state.accepting) {
      ...
      try {
        ...
        const modeNote = modeParts.length
          ? ` ${modeParts.join("/")} only.`
          : "";
        chatAnnouncement = await botReply("requestsOpenAnnouncement", {
          hunt: huntLabel,
          modeNote,
        });
        if (String(chatAnnouncement || "").trim()) {
