...
import {
  getSlotRequestState,
  setSlotRequestsAffiliatesAndSubsOnly,
  setSlotRequestsOpen,
...
export async function handleBonusHuntRequestsList(req, res) {
  try {
    ...
    const payload = {
      ...
      affiliatesAndSubsOnly: Boolean(requestState.affiliatesAndSubsOnly),
  ...
  try {
    ...
    if (state.accepting) {
      ...
      try {
        ...
        const modeNote = state.affiliatesAndSubsOnly
          ? " AFF and Kick subs only."
          : "";
    ...
    sendJson(res, 200, {
      ...
      affiliatesAndSubsOnly: Boolean(state.affiliatesAndSubsOnly),
      kickChatSubscribed,
      kickChatError,
      chatAnnouncement,
      webhookUrl: getKickWebhookUrl(req),
...
export async function handleBonusHuntRequestsAffSubOnly(req, res) {
  ...
  if (typeof body.affiliatesAndSubsOnly !== "boolean") {
    sendJson(res, 400, {
      error: "Provide affiliatesAndSubsOnly as true or false.",
    });
    return;
  }

  try {
    const state = await setSlotRequestsAffiliatesAndSubsOnly(
      body.affiliatesAndSubsOnly
    );
    sendJson(res, 200, {
      ok: true,
      acceptingRequests: Boolean(state.accepting),
