...
const DEFAULT_CONFIG = {
  ...
  alerts: {
    ...
    pointsMessage: "{user} earned {amount} points!",
  },
};
...
export function normalizeOverlayConfig(raw) {
  ...
  return {
    ...
    alerts: {
      ...
      pointsMessage:
        trimString(raw.alerts?.pointsMessage, 120) ||
        DEFAULT_CONFIG.alerts.pointsMessage,
    },
  };
}
...
function formatAlertMessage(template, { user, count, amount }) {
  return String(template || "")
    .replaceAll("{user}", user || "Someone")
    .replaceAll("{count}", String(count ?? 1))
    .replaceAll("{amount}", String(amount ?? count ?? 0));
}
...
export async function enqueueOverlayAlert({
  ...
  if (!resolvedMessage) {
    if (type === "sub") {
      ...
      resolvedMessage = formatAlertMessage(config.alerts.pointsMessage, {
        user: username,
        amount: amount ?? count,
        count: amount ?? count,
      });
