...
import {
  ...
  resetWeeklyRaffleWeek,
  revealWeeklyRaffleWinner,
  startWeeklyRaffleWeek,
  TICKETS_PER_DOLLARS,
  WEEK_DURATION_MS,
...
async function buildStatusPayload() {
  ...
  return {
    ...
    weekDurationMs: WEEK_DURATION_MS,
...
export async function handleWeeklyRaffleStartWeek(req, res) {
  const session = await getSession(req);
  if (!requireAdmin(session, res)) return;

  try {
    await startWeeklyRaffleWeek();
    const payload = await buildStatusPayload();
    sendJson(res, 200, payload);
  } catch (error) {
    sendJson(res, 500, {
      error: error.message || "Could not start the raffle week.",
    });
  }
}

export async function handleWeeklyRaffleResetWeek(req, res) {
  const session = await getSession(req);
  if (!requireAdmin(session, res)) return;

  try {
    await resetWeeklyRaffleWeek();
    const payload = await buildStatusPayload();
    sendJson(res, 200, payload);
  } catch (error) {
    sendJson(res, 500, {
      error: error.message || "Could not reset the raffle week.",
    });
  }
}
