...
async function writeRedisValue(key, value, ttlSeconds) {
  ...
  // Official Upstash REST command form — KV-style { value, ex } does not persist here.
...
/** Valid + unexpired only. Allows retries — Stake/browser may POST import-sync twice. */
export async function assertStakeSyncToken(token) {
  const record = await readToken(token);
  if (!record) {
    throw new Error("Sync token is invalid or expired. Start sync again from bonus-hunt.");
  }
...
export async function updateStakeSyncProgress(
  token,
  { count, withThumbnails = 0, progress = "" } = {}
) {
  const record = await readToken(token);
