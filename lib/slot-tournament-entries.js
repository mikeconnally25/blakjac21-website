...
export async function addSlotTournamentBotEntries(count) {
  const requested = Math.floor(Number(count));
  if (!Number.isFinite(requested) || requested < 1) {
    throw new Error("Add at least 1 bot.");
  }
  if (requested > 100) {
    throw new Error("You can add at most 100 bots at a time.");
  }
  ...
  const existingBotNumbers = new Set(
    store.entries
      ...
      .map((entry) => {
        const match = String(entry.username || "").match(/^Bot\s+(\d+)$/i);
  ...
  for (let i = 0; i < requested; i += 1) {
    while (existingBotNumbers.has(nextBotNumber)) {
      nextBotNumber += 1;
    }
    existingBotNumbers.add(nextBotNumber);

    const entry = {
      id: crypto.randomUUID(),
      kickUserId: `bot:${crypto.randomUUID()}`,
      username: `Bot ${nextBotNumber}`,
      enteredAt: new Date().toISOString(),
      isBot: true,
    };
    store.entries.push(entry);
    added.push(toPublicEntry(entry));
    nextBotNumber += 1;
  }
...
export async function removeSlotTournamentEntry(entryId) {
