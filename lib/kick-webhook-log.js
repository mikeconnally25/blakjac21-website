...
/** Persist ~1 in N successful webhook logs to Redis (errors always persist). */
const SUCCESS_SAMPLE_RATE = 40;
...
function shouldPersistToRedis(entry) {
  if (entry?.ok === false || entry?.error) {
    return true;
  }
  // Successful chat/command traffic is noisy — keep only a sample in Redis.
  return Math.floor(Math.random() * SUCCESS_SAMPLE_RATE) === 0;
}
...
export async function appendKickWebhookLog(entry) {
  ...
  if (!shouldPersistToRedis(record)) {
    return record;
  }
