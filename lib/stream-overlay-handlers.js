...
export async function handleStreamOverlayAlertsTest(req, res) {
  ...
  const allowed = new Set(["sub", "resub", "gift", "points", "test"]);
  const alertType = allowed.has(type) ? type : "sub";

  try {
    const alert = await enqueueOverlayAlert({
      type: alertType,
      username,
      count: alertType === "gift" ? count : null,
      amount: alertType === "points" ? amount : null,
      message:
        alertType === "test" ? `${username} triggered a test alert` : null,
    });
