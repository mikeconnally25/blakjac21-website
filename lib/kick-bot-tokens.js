...
function hasKickChatCredentials() {
  const clientId = String(process.env.KICK_CLIENT_ID || "").trim();
  const clientSecret = String(process.env.KICK_CLIENT_SECRET || "").trim();
  const broadcasterUserId = String(process.env.KICK_BROADCASTER_USER_ID || "").trim();

  return Boolean(clientId && clientSecret && broadcasterUserId);
}
