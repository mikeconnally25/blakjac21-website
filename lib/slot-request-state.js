...
function normalizeState(raw) {
  if (!raw || typeof raw !== "object") {
    return { accepting: false, affiliatesAndSubsOnly: false };
  }

  return {
    accepting: Boolean(raw.accepting),
    affiliatesAndSubsOnly: Boolean(raw.affiliatesAndSubsOnly),
  };
}
...
async function readFileState() {
  ...
  try {
    ...
    await fs.writeFile(
      STATE_FILE,
      JSON.stringify({ accepting: false, affiliatesAndSubsOnly: false }, null, 2)
    );
  }
...
export async function setSlotRequestsOpen(accepting) {
  ...
  const state = {
    ...
    affiliatesAndSubsOnly: Boolean(current.affiliatesAndSubsOnly),
  };
  await writeState(state);
  return state;
}

export async function setSlotRequestsAffiliatesAndSubsOnly(affiliatesAndSubsOnly) {
  const current = await readState();
  const state = {
    accepting: Boolean(current.accepting),
    affiliatesAndSubsOnly: Boolean(affiliatesAndSubsOnly),
  };
  await writeState(state);
  return state;
}
