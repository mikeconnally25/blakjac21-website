...
import { listKickChatArchiveForUser } from "./kick-chat-archive.js";
import { getUserByKickId } from "./users.js";
...
async function toPublicWinner(winner) {
  if (!winner) return null;

  let stakeUsername = null;
  const kickUserId = String(winner.kickUserId || "").trim();
  if (kickUserId) {
    try {
...
async function listWinnerPanelMessages(winner) {
  if (!winner) {
    return [];
  }

  const kickUserId = String(winner.kickUserId || "").trim();
  const archived = kickUserId
    ? await listKickChatArchiveForUser(kickUserId, {
        username: winner.username,
        days: 7,
      })
    : [];
  const live = await listWinnerChatMessages();

  const byId = new Map();
  for (const message of [...archived, ...live]) {
...
async function buildStatusPayload(session, extras = {}) {
  ...
  return {
    ...
    entries,
    winner: await toPublicWinner(state.winner),
    viewerIsWinner: viewerWins,
    canSeeWinnerChat,
    winnerMessages:
      canSeeWinnerChat && state.winner
        ? await listWinnerPanelMessages(state.winner)
        : [],
    ...extras,
  };
}
