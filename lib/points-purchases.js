...
export function resolvePurchaseAmount({ packageId, points, usd } = {}) {
  ...
  // Custom amounts are disabled — only the listed package is available.
  if (
    (points != null && String(points).trim() !== "") ||
    (usd != null && String(usd).trim() !== "")
  ) {
    throw new Error(
      "Custom amounts are not available. Choose the 2,000 points package."
    );
  }

  throw new Error("Choose the 2,000 points package to continue.");
}
