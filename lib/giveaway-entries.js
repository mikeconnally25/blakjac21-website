...
export async function addGiveawayEntry({ kickUserId, username }) {
  ...
  const { assertCanParticipate } = await import("./users.js");
...
export async function pickRandomGiveawayEntry() {
  ...
  // Dynamic import avoids a circular dependency with users.js.
  ...
  const eligible = entries.filter((entry) => {
    ...
    if (softBlockIndex.get(kickUserId)?.softBlocked) {
      return false;
    }
    return stakeLinkedIds.has(kickUserId);
  });

  // Also drop permanently banned accounts (manual or alt) even if they somehow entered.
  const stillEligible = [];
  for (const entry of eligible) {
    const user = await getUserByKickId(entry.kickUserId);
  ...
  if (!stillEligible.length) {
    throw new Error(
      "No eligible entrants to reveal. Winners need a linked Stake account, and blocked/alt accounts are excluded."
    );
  }

  return stillEligible[crypto.randomInt(0, stillEligible.length)];
}
