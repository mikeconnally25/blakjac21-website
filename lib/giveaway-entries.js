...
export async function pickRandomGiveawayEntry() {
  ...
  const { getAltSoftBlockIndex, getStakeLinkedKickUserIds } = await import(
    "./users.js"
  );
  ...
  if (!eligible.length) {
    throw new Error(
      "No eligible entrants to reveal. Winners need a linked Stake account, and newer similar-IP alts are soft-blocked."
    );
  }
