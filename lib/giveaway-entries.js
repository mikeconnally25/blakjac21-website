...
export async function pickRandomGiveawayEntry() {
  ...
  const { getAltSoftBlockIndex } = await import("./users.js");
  ...
  const eligible = entries.filter((entry) => {
    ...
    return !softBlockIndex.get(kickUserId)?.softBlocked;
  });

  if (!eligible.length) {
    throw new Error(
      "No eligible entrants to reveal. Every entrant is soft-blocked as a newer shared-IP alt — only the oldest account in each IP cluster can win."
    );
  }
