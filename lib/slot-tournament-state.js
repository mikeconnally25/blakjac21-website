...
export function normalizeTournamentKeyword(raw) {
...
function normalizeState(raw) {
  ...
  let keyword = "";
  try {
    keyword = normalizeTournamentKeyword(raw.keyword);
  } catch {
    keyword = "";
  }
...
export async function setSlotTournamentOpen(open) {
  ...
  if (nextOpen && !current.keyword) {
    throw new Error("Set a chat keyword before opening signups.");
  }
...
export async function setSlotTournamentSettings({
  ...
  const nextKeyword =
    keyword === undefined
      ? current.keyword
      : normalizeTournamentKeyword(keyword);

  if (current.open && !nextKeyword) {
    throw new Error("Cannot clear keyword while signups are open.");
  }
