...
function slotKeysMatch(slot, { name, slug }) {
  const slotSlug = String(slot?.slug || "").trim().toLowerCase();
  const nextSlug = String(slug || "").trim().toLowerCase();
  if (slotSlug && nextSlug && slotSlug === nextSlug) {
    return true;
  }
  const slotName = String(slot?.name || "").trim().toLowerCase();
  const nextName = String(name || "").trim().toLowerCase();
  return Boolean(slotName && nextName && slotName === nextName);
}

/**
 * Assign a catalog slot to an entrant. Enforces unique slug/name and one slot per entry.
 */
export async function claimSlotForEntry({ entryId, slot } = {}) {
  const targetEntryId = String(entryId || "").trim();
  if (!targetEntryId) {
    throw new Error("Entry id is required.");
  }

  const name = String(slot?.name || "").trim().slice(0, TITLE_MAX_LENGTH);
  const slug = String(slot?.slug || "").trim().slice(0, 120) || null;
  const thumbnailUrl =
    String(slot?.thumbnailUrl || "").trim().slice(0, 500) || null;
  ...
  const current = await readState();
  const taken = current.slots.some((existing) => {
    if (String(existing.entryId || "").trim() === targetEntryId) {
      return false;
    }
    if (!existing.entryId) {
      return false;
    }
    return slotKeysMatch(existing, { name, slug });
  });
  if (taken) {
    throw new Error("That slot is already taken.");
  }

  const withoutOwn = current.slots.filter(
    (existing) => String(existing.entryId || "").trim() !== targetEntryId
  );

  const reusableIndex = withoutOwn.findIndex(
    (existing) => !existing.entryId && slotKeysMatch(existing, { name, slug })
  );

  let nextSlots;
  if (reusableIndex >= 0) {
    nextSlots = withoutOwn.map((existing, index) =>
      index === reusableIndex
        ? {
            ...existing,
            name,
  ...
  return writeState({
    ...current,
    slots: normalizeTournamentSlots(nextSlots),
  });
}
...
/** Drop slot rows claimed by this entrant (or just unassign if shared). */
export async function releaseSlotsForEntry(entryId) {
  const targetEntryId = String(entryId || "").trim();
  if (!targetEntryId) {
    throw new Error("Entry id is required.");
  }
  ...
  const nextSlots = current.slots.filter(
    (slot) => String(slot.entryId || "").trim() !== targetEntryId
  );
