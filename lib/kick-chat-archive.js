...
const ARCHIVE_WRITE_MIN_MS = 30_000;
const EXPIRE_REFRESH_MS = 10 * 60 * 1000;

/** In-memory throttle for archive writes (per warm serverless instance). */
const lastArchiveWriteAt = new Map();
let lastRecentChattersExpireAt = 0;

function rememberArchiveWrite(userId) {
  lastArchiveWriteAt.set(userId, Date.now());
  if (lastArchiveWriteAt.size > 800) {
    const cutoff = Date.now() - ARCHIVE_WRITE_MIN_MS * 4;
    for (const [id, at] of lastArchiveWriteAt) {
      if (at < cutoff) lastArchiveWriteAt.delete(id);
    }
  }
}

function shouldSkipArchiveWrite(userId) {
  const last = lastArchiveWriteAt.get(userId) || 0;
  return Date.now() - last < ARCHIVE_WRITE_MIN_MS;
}
...
async function touchRecentChatter({ kickUserId, username, at }) {
  ...
  if (config) {
    ...
    // Refresh TTL occasionally instead of on every chat message.
    const now = Date.now();
    if (now - lastRecentChattersExpireAt >= EXPIRE_REFRESH_MS) {
      await redisCommand(config, [
        "EXPIRE",
        RECENT_CHATTERS_HASH_KEY,
        ttlSeconds,
      ]);
      lastRecentChattersExpireAt = now;
    }
    return;
  }
...
export async function appendKickChatArchiveMessage({
  ...
  // Throttle full per-user archive GET+SET during chat spam.
  if (shouldSkipArchiveWrite(userId)) {
    return message;
  }
