import { getSession } from "./session.js";
import {
  clearSlotTournamentBracket,
  clearSlotTournamentSlotAssignments,
  generateSlotTournamentBracket,
  getSlotTournamentState,
  releaseSlotsForEntry,
  setSlotTournamentAffiliatesOnly,
  setSlotTournamentBracketScore,
  setSlotTournamentOpen,
  setSlotTournamentPhase,
  setSlotTournamentResults,
  setSlotTournamentSettings,
  setSlotTournamentSlots,
  claimSlotForEntry,
  setSlotTournamentSubscribersOnly,
} from "./slot-tournament-state.js";
import {
  addSlotTournamentBotEntries,
  addSlotTournamentEntry,
  clearSlotTournamentEntries,
  countSlotTournamentEntries,
  findSlotTournamentEntryById,
  listSlotTournamentEntries,
  removeSlotTournamentEntry,
} from "./slot-tournament-entries.js";
import {
  areSlotTournamentPredictionsOpen,
  clearSlotTournamentPredictions,
  findSlotTournamentPrediction,
  getSlotTournamentPredictionTimeRemainingMs,
  getSlotTournamentPredictionsState,
  saveSlotTournamentPrediction,
  setSlotTournamentPredictionsOpen,
  toPublicPrediction,
  toPublicPredictionLeaderboard,
  toPublicPredictors,
} from "./slot-tournament-predictions.js";
import {
  ensureKickChatSubscription,
  listKickChatSubscriptions,
} from "./kick-chat.js";
import { getAllowedSlotCatalog } from "./stake-slots.js";
import { isActiveKickSubscriber } from "./kick-subscribers.js";
import { findUserByUsername, getUserByKickId, getUsersByKickIds, withLiveUserBadges } from "./users.js";
import { findPointsBalanceByUsername } from "./points.js";

function sendJson(res, statusCode, payload) {
  res.statusCode = statusCode;
  res.setHeader("Content-Type", "application/json");
  res.end(JSON.stringify(payload));
}

async function assertTournamentAccess(state, session) {
  if (!state.affiliatesOnly && !state.subscribersOnly) {
    return;
  }

  const storedUser = await getUserByKickId(session.kickUserId);
  const user = await withLiveUserBadges({
    kickUserId: session.kickUserId,
    username: storedUser?.username || session.username,
    stakeUsername: storedUser?.stakeUsername ?? null,
    stakeLinkedAt: storedUser?.stakeLinkedAt ?? null,
    stakeCodeVerified: storedUser?.stakeCodeVerified ?? false,
    stakeCodeVerifiedAt: storedUser?.stakeCodeVerifiedAt ?? null,
    affGranted: Boolean(storedUser?.affGranted),
  });

  const isAffiliate = Boolean(user.stakeCodeVerified);
  const isSubscriber =
    Boolean(user.kickSubActive) ||
    (await isActiveKickSubscriber(session.kickUserId, session.username));

  let allowed = false;
  if (state.affiliatesOnly && state.subscribersOnly) {
    allowed = isAffiliate || isSubscriber;
  } else if (state.affiliatesOnly) {
    allowed = isAffiliate;
  } else {
    allowed = isSubscriber;
  }

  if (allowed) return;

  if (state.affiliatesOnly && !state.subscribersOnly) {
    throw new Error(
      "AFF only — link Stake with code BLAKJAC21 before claiming a spot."
    );
  }
  if (state.subscribersOnly && !state.affiliatesOnly) {
    throw new Error("SUB only — you need an active Kick subscription.");
  }
  throw new Error(
    "This tournament is restricted to verified AFF or active Kick subs."
  );
}

async function readJsonBody(req) {
  if (typeof req.body === "string" && req.body.trim()) {
    return JSON.parse(req.body);
  }

  if (req.body && typeof req.body === "object") {
    return req.body;
  }

  const chunks = [];
  for await (const chunk of req) {
    chunks.push(chunk);
  }

  const raw = Buffer.concat(chunks).toString();
  if (!raw) return {};
  return JSON.parse(raw);
}

function hasChatMessageSubscription(subscriptions) {
  return (subscriptions || []).some(
    (entry) =>
      String(entry.event || entry.name || "").toLowerCase() === "chat.message.sent"
  );
}

async function buildStatusPayload(session, extras = {}) {
  const state = await getSlotTournamentState();
  const isAdmin = Boolean(session?.isAdmin);
  const allEntries = await listSlotTournamentEntries({ includePrivate: true });
  const predictionState = await getSlotTournamentPredictionsState();
  const predictionsOpen = areSlotTournamentPredictionsOpen(predictionState);
  const predictionsRemainingMs =
    getSlotTournamentPredictionTimeRemainingMs(predictionState);
  const entries = isAdmin
    ? allEntries
    : allEntries.map((entry) => ({
        id: entry.id,
        username: entry.username,
        enteredAt: entry.enteredAt,
        isBot: Boolean(entry.isBot),
      }));

  const viewerKickId = String(session?.kickUserId || "").trim();
  const viewerEntry =
    viewerKickId
      ? allEntries.find((entry) => entry.kickUserId === viewerKickId) || null
      : null;
  const viewerEntered = Boolean(viewerEntry);
  const viewerPredictionRecord = viewerKickId
    ? await findSlotTournamentPrediction(viewerKickId)
    : null;
  const bracket = state.bracket || {
    generatedAt: null,
    entrantIds: [],
    matches: [],
  };
  const predictionLeaderboard = toPublicPredictionLeaderboard(
    predictionState.predictions,
    bracket.matches
  );
  const predictors = toPublicPredictors(predictionState.predictions);

  const profileByKickId = new Map();
  const leaderboardKickIds = predictionLeaderboard
    .map((row) => String(row.kickUserId || "").trim())
    .filter(Boolean);
  const usersByKickId = await getUsersByKickIds(leaderboardKickIds);
  for (const kickUserId of leaderboardKickIds) {
    if (profileByKickId.has(kickUserId)) continue;
    const user = usersByKickId.get(kickUserId);
    profileByKickId.set(kickUserId, {
      stakeUsername: String(user?.stakeUsername || "").trim() || null,
      isAffiliate: Boolean(user?.stakeCodeVerified || user?.affGranted),
    });
  }
  const enrichedPredictionLeaderboard = predictionLeaderboard.map((row) => {
    const profile =
      profileByKickId.get(String(row.kickUserId || "").trim()) || {};
    return {
      ...row,
      stakeUsername: profile.stakeUsername || null,
      isAffiliate: Boolean(profile.isAffiliate),
    };
  });

  return {
    open: Boolean(state.open),
    phase: state.phase,
    title: state.title,
    slotName: state.slotName,
    buyIn: state.buyIn,
    capacity: Number(state.capacity) || 0,
    spotsLeft: Math.max(0, (Number(state.capacity) || 0) - allEntries.length),
    affiliatesOnly: Boolean(state.affiliatesOnly),
    subscribersOnly: Boolean(state.subscribersOnly),
    entryCount: allEntries.length,
    entries,
    slots: Array.isArray(state.slots) ? state.slots : [],
    bracket,
    results: state.results || [],
    predictionsOpen,
    predictionsEndsAt: predictionsOpen ? predictionState.endsAt || null : null,
    predictionsSecondsRemaining: predictionsOpen
      ? Math.ceil(predictionsRemainingMs / 1000)
      : 0,
    predictionsTimerMinutes: Number(predictionState.timerMinutes) || 5,
    viewerPrediction: toPublicPrediction(viewerPredictionRecord),
    predictionLeaderboard: enrichedPredictionLeaderboard,
    predictors,
    predictionCount: predictors.length,
    viewerEntered,
    viewerEntryId: viewerEntry?.id || null,
    updatedAt: state.updatedAt,
    ...extras,
  };
}

export async function handleSlotTournamentStatus(req, res) {
  try {
    const session = await getSession(req);
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 500, {
      error: error.message || "Could not load tournament status.",
    });
  }
}

export async function handleSlotTournamentToggle(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  if (typeof body.open !== "boolean") {
    sendJson(res, 400, { error: "Provide open as true or false." });
    return;
  }

  try {
    const state = await setSlotTournamentOpen(body.open);
    let kickChatSubscribed = false;
    let kickChatError = null;

    if (state.open) {
      await clearSlotTournamentEntries();
      await clearSlotTournamentPredictions();
      try {
        await ensureKickChatSubscription();
        const { subscriptions, error } = await listKickChatSubscriptions();
        kickChatSubscribed = hasChatMessageSubscription(subscriptions);
        kickChatError = error || null;
      } catch (error) {
        kickChatError = error.message;
        console.error("Kick chat subscription failed:", error.message);
      }
    }

    sendJson(res, 200, {
      ...(await buildStatusPayload(session)),
      kickChatSubscribed,
      kickChatError,
    });
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not update tournament signups.",
    });
  }
}

export async function handleSlotTournamentSettings(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    await setSlotTournamentSettings({
      title: body.title,
      slotName: body.slotName,
      buyIn: body.buyIn,
      capacity: body.capacity,
    });
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not save tournament settings.",
    });
  }
}

export async function handleSlotTournamentPhase(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    await setSlotTournamentPhase(body.phase);
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not update tournament phase.",
    });
  }
}

export async function handleSlotTournamentAffiliatesOnly(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    await setSlotTournamentAffiliatesOnly(Boolean(body.affiliatesOnly));
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not update AFF setting.",
    });
  }
}

export async function handleSlotTournamentSubscribersOnly(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    await setSlotTournamentSubscribersOnly(Boolean(body.subscribersOnly));
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not update SUB setting.",
    });
  }
}

export async function handleSlotTournamentEntriesClear(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  try {
    await clearSlotTournamentEntries();
    await clearSlotTournamentSlotAssignments();
    await clearSlotTournamentBracket();
    await clearSlotTournamentPredictions();
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not clear entries.",
    });
  }
}

export async function handleSlotTournamentEntryRemove(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const entryId = String(body.entryId || "").trim();
    if (!entryId) {
      sendJson(res, 400, { error: "Entry id is required." });
      return;
    }

    const state = await getSlotTournamentState();
    const inBracket = (state.bracket?.entrantIds || []).includes(entryId);
    const result = await removeSlotTournamentEntry(entryId);
    await releaseSlotsForEntry(entryId);

    if (inBracket) {
      await clearSlotTournamentBracket();
      await clearSlotTournamentPredictions();
    }

    sendJson(res, 200, {
      ...(await buildStatusPayload(session)),
      removedEntryId: result.entry?.id || entryId,
      username: result.entry?.username || null,
      bracketCleared: inBracket,
    });
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not remove entrant.",
    });
  }
}

export async function handleSlotTournamentEntryAdd(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const username = String(body.username || "")
      .replace(/^@/, "")
      .trim();
    if (!username) {
      sendJson(res, 400, { error: "Enter a Kick username." });
      return;
    }

    const state = await getSlotTournamentState();
    const capacity = Number(state.capacity) || 0;
    const currentCount = await countSlotTournamentEntries();
    if (capacity && currentCount >= capacity) {
      sendJson(res, 400, { error: "Tournament is already full." });
      return;
    }

    const allEntries = await listSlotTournamentEntries({ includePrivate: true });
    const nameKey = username.toLowerCase();
    const alreadyNamed = allEntries.find(
      (entry) => String(entry.username || "").trim().toLowerCase() === nameKey
    );
    if (alreadyNamed) {
      sendJson(res, 400, {
        error: `@${alreadyNamed.username} is already signed up.`,
      });
      return;
    }

    let kickUserId = "";
    let displayName = username;

    const signedIn = await findUserByUsername(username);
    if (signedIn?.kickUserId) {
      kickUserId = String(signedIn.kickUserId);
      displayName = signedIn.username || username;
    } else {
      const balanceHit = await findPointsBalanceByUsername(username);
      if (balanceHit?.kickUserId) {
        kickUserId = String(balanceHit.kickUserId);
        displayName = balanceHit.username || username;
      }
    }

    if (!kickUserId) {
      kickUserId = `manual:${nameKey}`;
    }

    const alreadyId = allEntries.find(
      (entry) => String(entry.kickUserId || "") === kickUserId
    );
    if (alreadyId) {
      sendJson(res, 400, {
        error: `@${alreadyId.username} is already signed up.`,
      });
      return;
    }

    const result = await addSlotTournamentEntry({
      kickUserId,
      username: displayName,
    });

    if (result.alreadyEntered) {
      sendJson(res, 400, {
        error: `@${result.entry?.username || displayName} is already signed up.`,
      });
      return;
    }

    const nextCount = await countSlotTournamentEntries();
    if (capacity && nextCount >= capacity && state.open) {
      await setSlotTournamentOpen(false);
    }

    sendJson(res, 200, {
      ...(await buildStatusPayload(session)),
      addedEntryId: result.entry?.id || null,
      username: result.entry?.username || displayName,
    });
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not add entrant.",
    });
  }
}

export async function handleSlotTournamentEntriesBots(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const state = await getSlotTournamentState();
    const currentCount = await countSlotTournamentEntries();
    const capacity = Number(state.capacity) || 0;
    const spotsLeft = capacity
      ? Math.max(0, capacity - currentCount)
      : 100;

    if (capacity && spotsLeft <= 0) {
      sendJson(res, 400, { error: "Tournament is already full." });
      return;
    }

    let count = Math.floor(Number(body.count));
    if (body.fillRemaining) {
      count = spotsLeft;
    }

    if (!Number.isFinite(count) || count < 1) {
      sendJson(res, 400, {
        error: capacity
          ? "Enter how many bots to add, or fill remaining spots."
          : "Enter how many bots to add.",
      });
      return;
    }

    if (capacity) {
      count = Math.min(count, spotsLeft);
    }

    const result = await addSlotTournamentBotEntries(count);
    let slotsAssigned = 0;
    try {
      slotsAssigned = await assignCatalogSlotsToEntries(result.added || []);
    } catch (error) {
      console.error("Bot slot assignment failed:", error.message);
    }

    if (capacity) {
      const nextCount = await countSlotTournamentEntries();
      if (nextCount >= capacity && state.open) {
        await setSlotTournamentOpen(false);
      }
    }

    sendJson(res, 200, {
      ...(await buildStatusPayload(session)),
      botsAdded: result.count,
      slotsAssigned,
    });
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not add bots.",
    });
  }
}

export async function handleSlotTournamentBracketGenerate(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body = {};
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const entries = await listSlotTournamentEntries({ includePrivate: true });
    await generateSlotTournamentBracket(entries, {
      force: Boolean(body.force),
    });
    await clearSlotTournamentPredictions();
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not generate bracket.",
    });
  }
}

export async function handleSlotTournamentBracketScore(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    await setSlotTournamentBracketScore({
      matchId: body.matchId,
      scoreA: body.scoreA,
      scoreB: body.scoreB,
    });
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not save match score.",
    });
  }
}

export async function handleSlotTournamentBracketClear(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  try {
    await clearSlotTournamentBracket();
    await clearSlotTournamentPredictions();
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not clear bracket.",
    });
  }
}

export async function handleSlotTournamentPredictionsToggle(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  if (typeof body.open !== "boolean") {
    sendJson(res, 400, { error: "Provide open as true or false." });
    return;
  }

  try {
    if (body.open) {
      const state = await getSlotTournamentState();
      if (!state.bracket?.matches?.length) {
        sendJson(res, 400, {
          error: "Generate a bracket before enabling predictions.",
        });
        return;
      }
      if (body.minutes === null || body.minutes === undefined) {
        sendJson(res, 400, {
          error: "Set a prediction timer length in minutes.",
        });
        return;
      }
    }

    await setSlotTournamentPredictionsOpen(
      body.open,
      body.open ? body.minutes : null
    );
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not update predictions.",
    });
  }
}

export async function handleSlotTournamentPredictionsSave(req, res) {
  const session = await getSession(req);
  if (!session?.kickUserId) {
    sendJson(res, 401, { error: "Sign in with Kick to submit predictions." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const state = await getSlotTournamentState();
    const matches = Array.isArray(state.bracket?.matches)
      ? state.bracket.matches
      : [];
    if (!matches.length) {
      sendJson(res, 400, { error: "No bracket is available to predict." });
      return;
    }

    const matchById = new Map(matches.map((match) => [match.id, match]));
    const validEntryIds = new Set(
      [
        ...(Array.isArray(state.bracket?.entrantIds)
          ? state.bracket.entrantIds
          : []),
        ...matches.flatMap((match) => [match.entryAId, match.entryBId, match.winnerEntryId]),
      ]
        .map((id) => String(id || "").trim())
        .filter(Boolean)
    );
    const rawPicks =
      body.picks && typeof body.picks === "object" && !Array.isArray(body.picks)
        ? body.picks
        : {};
    const picks = {};

    for (const [matchId, winnerEntryId] of Object.entries(rawPicks)) {
      const match = matchById.get(String(matchId || "").trim());
      const winner = String(winnerEntryId || "").trim();
      if (!match || !winner) {
        continue;
      }
      if (!validEntryIds.has(winner)) {
        throw new Error("Pick must be a tournament entrant.");
      }
      // Round 1 must pick one of the seeded players when both sides are set.
      if (
        Number(match.round) === 1 &&
        match.entryAId &&
        match.entryBId &&
        winner !== match.entryAId &&
        winner !== match.entryBId
      ) {
        throw new Error("Pick must be one of the matchup players.");
      }
      picks[match.id] = winner;
    }

    await saveSlotTournamentPrediction({
      kickUserId: session.kickUserId,
      username: session.username,
      picks,
    });
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not save predictions.",
    });
  }
}

export async function handleSlotTournamentSlots(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const entries = await listSlotTournamentEntries({ includePrivate: true });
    await setSlotTournamentSlots(body.slots, {
      validEntryIds: entries.map((entry) => entry.id),
    });
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not save slot assignments.",
    });
  }
}

export async function handleSlotTournamentChangeEntrySlot(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const entryId = String(body.entryId || "").trim();
    if (!entryId) {
      sendJson(res, 400, { error: "Entry id is required." });
      return;
    }

    const entry = await findSlotTournamentEntryById(entryId);
    if (!entry) {
      sendJson(res, 404, { error: "Entrant not found." });
      return;
    }

    const catalogSlot = await resolveCatalogSlotFromBody(body);
    const state = await getSlotTournamentState();
    if (slotAlreadyTaken(state.slots, catalogSlot, entryId)) {
      sendJson(res, 400, { error: "That slot is already taken." });
      return;
    }

    await claimSlotForEntry({ entryId, slot: catalogSlot });
    sendJson(res, 200, {
      ...(await buildStatusPayload(session)),
      changedEntryId: entryId,
      slotName: catalogSlot.name,
    });
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not change slot.",
    });
  }
}

export async function handleSlotTournamentResults(req, res) {
  const session = await getSession(req);
  if (!session?.isAdmin) {
    sendJson(res, 403, { error: "Admin access required." });
    return;
  }

  let body;
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const rows = Array.isArray(body.results) ? body.results : [];
    const resolved = [];

    for (const row of rows) {
      let username = String(row?.username || "").trim();
      let kickUserId = String(row?.kickUserId || "").trim() || null;
      const entryId = String(row?.entryId || "").trim();

      if (entryId) {
        const entry = await findSlotTournamentEntryById(entryId);
        if (entry) {
          username = entry.username || username;
          kickUserId = entry.kickUserId || kickUserId;
        }
      }

      resolved.push({
        place: row?.place,
        entryId: entryId || null,
        kickUserId,
        username,
        score: row?.score,
        note: row?.note,
      });
    }

    await setSlotTournamentResults(resolved);
    sendJson(res, 200, await buildStatusPayload(session));
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not save results.",
    });
  }
}

function normalizeThumbnailUrl(url) {
  const value = String(url || "").trim();
  if (!value) return null;
  const normalized = value.startsWith("//") ? `https:${value}` : value;
  return normalized.slice(0, 500);
}

async function resolveCatalogSlotFromBody(body = {}) {
  const slug = String(body.slotSlug || body.slug || "").trim().toLowerCase();
  const nameNeedle = String(body.slotName || body.name || "")
    .trim()
    .toLowerCase();

  if (!slug && !nameNeedle) {
    throw new Error("Pick a slot to claim your spot.");
  }

  const catalog = await getAllowedSlotCatalog();
  const slots = Array.isArray(catalog?.slots) ? catalog.slots : [];
  const hit =
    slots.find(
      (slot) => slug && String(slot.slug || "").trim().toLowerCase() === slug
    ) ||
    slots.find(
      (slot) =>
        nameNeedle && String(slot.name || "").trim().toLowerCase() === nameNeedle
    );

  if (!hit?.name) {
    throw new Error(
      "Choose a slot from New Releases or Only on Stake on stake.com."
    );
  }

  return {
    name: String(hit.name).trim(),
    slug: String(hit.slug || "").trim() || null,
    thumbnailUrl: normalizeThumbnailUrl(hit.thumbnailUrl),
  };
}

function slotAlreadyTaken(slots, { name, slug }, exceptEntryId = null) {
  const except = String(exceptEntryId || "").trim();
  return (slots || []).some((existing) => {
    const owner = String(existing.entryId || "").trim();
    if (!owner || (except && owner === except)) return false;
    const existingSlug = String(existing.slug || "").trim().toLowerCase();
    const nextSlug = String(slug || "").trim().toLowerCase();
    if (existingSlug && nextSlug && existingSlug === nextSlug) return true;
    return (
      String(existing.name || "").trim().toLowerCase() ===
      String(name || "").trim().toLowerCase()
    );
  });
}

async function assignCatalogSlotsToEntries(entries) {
  if (!entries?.length) return 0;

  const catalog = await getAllowedSlotCatalog();
  const available = [...(catalog?.slots || [])];
  let state = await getSlotTournamentState();
  let assigned = 0;

  for (const entry of entries) {
    const entryId = String(entry.id || "").trim();
    if (!entryId) continue;
    if (
      state.slots.some((slot) => String(slot.entryId || "").trim() === entryId)
    ) {
      continue;
    }

    while (available.length) {
      const candidate = available.shift();
      const slot = {
        name: String(candidate?.name || "").trim(),
        slug: String(candidate?.slug || "").trim() || null,
        thumbnailUrl: normalizeThumbnailUrl(candidate?.thumbnailUrl),
      };
      if (!slot.name) continue;
      if (slotAlreadyTaken(state.slots, slot)) continue;
      try {
        state = await claimSlotForEntry({ entryId, slot });
        assigned += 1;
        break;
      } catch {
        // try next catalog slot
      }
    }
  }

  return assigned;
}

export async function handleSlotTournamentJoin(req, res) {
  const session = await getSession(req);
  if (!session?.kickUserId) {
    sendJson(res, 401, { error: "Sign in with Kick to claim a spot." });
    return;
  }

  let body = {};
  try {
    body = await readJsonBody(req);
  } catch {
    sendJson(res, 400, { error: "Invalid request body." });
    return;
  }

  try {
    const state = await getSlotTournamentState();
    if (!state.open) {
      sendJson(res, 400, { error: "Tournament signups are closed." });
      return;
    }

    if (!state.capacity) {
      sendJson(res, 400, { error: "Tournament capacity is not set." });
      return;
    }

    await assertTournamentAccess(state, session);

    const catalogSlot = await resolveCatalogSlotFromBody(body);
    const allEntries = await listSlotTournamentEntries({ includePrivate: true });
    const existing = allEntries.find(
      (entry) => entry.kickUserId === String(session.kickUserId)
    );

    if (existing) {
      const alreadyHasSlot = state.slots.some(
        (slot) => String(slot.entryId || "").trim() === existing.id
      );
      if (alreadyHasSlot) {
        sendJson(res, 200, {
          ...(await buildStatusPayload(session)),
          alreadyEntered: true,
        });
        return;
      }

      if (slotAlreadyTaken(state.slots, catalogSlot, existing.id)) {
        sendJson(res, 400, { error: "That slot is already taken." });
        return;
      }

      await claimSlotForEntry({ entryId: existing.id, slot: catalogSlot });
      sendJson(res, 200, {
        ...(await buildStatusPayload(session)),
        alreadyEntered: true,
        slotAssigned: true,
      });
      return;
    }

    const currentCount = allEntries.length;
    if (currentCount >= state.capacity) {
      await setSlotTournamentOpen(false);
      sendJson(res, 400, { error: "Tournament is full." });
      return;
    }

    if (slotAlreadyTaken(state.slots, catalogSlot)) {
      sendJson(res, 400, { error: "That slot is already taken." });
      return;
    }

    const result = await addSlotTournamentEntry({
      kickUserId: session.kickUserId,
      username: session.username,
    });

    try {
      await claimSlotForEntry({
        entryId: result.entry.id,
        slot: catalogSlot,
      });
    } catch (error) {
      // Entry was created; leave it so they can retry picking another slot.
      throw error;
    }

    const nextCount = await countSlotTournamentEntries();
    if (nextCount >= state.capacity) {
      await setSlotTournamentOpen(false);
    }

    sendJson(res, 200, {
      ...(await buildStatusPayload(session)),
      alreadyEntered: false,
    });
  } catch (error) {
    sendJson(res, 400, {
      error: error.message || "Could not claim a spot.",
    });
  }
}
