...
import { stripProviderFromSlotName } from "./stake-slots.js";
...
function normalizeBonus(raw) {
  ...
  const slot =
    stripProviderFromSlotName(raw.slot, provider) ||
    String(raw.slot || "").trim();
...
function normalizeStore(raw) {
  ...
  return {
    ...
    collecting: raw?.collecting !== false,
    bonuses,
  };
}
...
function bonusTierRank(bonus) {
  if (Boolean(bonus?.epicBonus)) {
    return 2;
  }
  if (Boolean(bonus?.superBonus)) {
    return 1;
  }
  return 0;
}
...
function sortBonuses(bonuses) {
  return [...bonuses].sort((a, b) => {
    const tierDiff = bonusTierRank(a) - bonusTierRank(b);
    if (tierDiff !== 0) {
      return tierDiff;
    }

    const betDiff = Number(b.bet || 0) - Number(a.bet || 0);
    if (betDiff !== 0) {
      return betDiff;
    }
...
function buildSummary(bonuses, { startBalance = 0 } = {}) {
  ...
  // Hunt profit vs starting balance: winnings minus start (buy-ins not counted).
  const profit = totalWon - normalizedStart;
  // Live tracker P/L: start − winnings (positive while still behind start).
...
export async function getBonusHunt() {
  ...
  return {
    hunt: {
      ...
      collecting: store.collecting !== false,
...
export async function addBonus({
  ...
  const store = await readStore();
  if (store.collecting === false) {
    throw new Error("Collecting is off. Turn Collecting back on to add bonuses.");
  }
...
export async function updateHuntSettings({
  title,
  startBalance,
  showHighestMulti,
  collecting,
