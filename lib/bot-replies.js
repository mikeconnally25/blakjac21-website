...
export const BOT_REPLY_FIELDS = [
  ...
  {
    key: "chatPromo",
    label: "Chat promo (every 10 min)",
    group: "Promo",
    description: "Posted automatically to Kick chat about every 10 minutes.",
  },
  {
    key: "commandsList",
    label: "!commands reply",
    group: "Channel",
    description: "Vars: {username} {url}",
  },
  {
    key: "shopReply",
    label: "!shop reply",
  ...
  {
    key: "shoutoutReply",
    label: "!so / !promo reply",
    group: "Channel",
    description: "Vars: {name} {url}",
  },
  {
    key: "shoutoutUsage",
    label: "!so usage",
  ...
  },
  {
    ...
    label: "!uptime — live",
    group: "Channel",
    description: "Vars: {time}",
  },
  {
    key: "uptimeOffline",
    label: "!uptime — offline",
    group: "Channel",
  },
  {
    key: "pointsBalance",
    ...
    group: "Points",
    description: "Vars: {username} {points}",
  },
  {
    key: "followageYes",
    label: "!followage — following",
    group: "Channel",
    description: "Vars: {username} {time}",
  },
  {
    key: "followageNo",
    label: "!followage — unknown / not following",
    group: "Channel",
    description: "Vars: {username}",
  },
  {
    key: "watchtimeReply",
    label: "!watchtime reply",
    group: "Channel",
    description: "Vars: {username} {time}",
  },
  {
    key: "levelReply",
    label: "!level — has level",
    group: "Channel",
    description: "Vars: {username} {level}",
  },
  {
    key: "levelNone",
    label: "!level — no level yet",
    group: "Channel",
    description: "Vars: {username}",
  },
  {
    ...
    label: "!xp — progress",
    group: "Channel",
    description: "Vars: {username} {xp} {nextlvl}",
  },
  {
    key: "xpNone",
    label: "!xp — no XP yet",
  ...
  {
    key: "addcomSuccess",
    label: "!addcom success",
    group: "Channel",
    description: "Vars: {trigger}",
  },
  {
    key: "addcomExists",
    label: "!addcom already exists",
    group: "Channel",
    description: "Vars: {trigger}",
  },
  {
  ...
  {
    key: "delcomSuccess",
    label: "!delcom success",
    group: "Channel",
    description: "Vars: {trigger}",
  },
  {
    key: "delcomMissing",
    label: "!delcom not found",
    group: "Channel",
    description: "Vars: {trigger}",
  },
  {
  ...
  {
    key: "editcomSuccess",
    label: "!editcom success",
    group: "Channel",
    description: "Vars: {trigger}",
  },
  {
    key: "editcomMissing",
    label: "!editcom not found",
    group: "Channel",
    description: "Vars: {trigger}",
  },
  {
    key: "commandAdminOnly",
    label: "Mod command — admin only",
    group: "Channel",
  },
];
...
export const DEFAULT_BOT_REPLIES = {
  slotUsage:
    "Use: !r <slot name> — up to 3 slots from New Releases or Only on Stake.",
  ...
  requestsOpenAnnouncement:
    "Requests are now open for hunt {hunt}, type !r to request a slot!{modeNote}",
  ...
  chatPromo:
    "Sign up on Stake.com today! https://stake.com/?offer=blakjac21&c=c52feb0e28 Need a kyc'd for stake account? Make a ticket in !discord for more information.",
  commandsList: "Command list: https://blakjac21.com/commands/",
  shopReply:
    "Hey @{username}, check out the channel shop: https://blakjac21.com/store/",
  topReply: "Top supporters list: https://blakjac21.com/leaderboard/",
  shoutoutReply: "Go follow {name} -> https://kick.com/{name}",
  shoutoutUsage: "Use: !so username",
  shoutoutAdminOnly: "Only admins can use shoutouts.",
  uptimeLive: "This stream started {time} ago.",
  uptimeOffline: "Stream is offline.",
  pointsBalance: "@{username} has {points} points.",
  followageYes:
    "@{username} has been following this channel for {time}!",
  followageNo: "@{username} hasn't followed this channel yet.",
  watchtimeReply: "@{username} has watched this channel for {time}.",
  levelReply: "@{username} is level {level}.",
  levelNone: "@{username} doesn't have a level yet.",
  xpReply:
    "@{username} has {xp} / {nextlvl} XP until the next level.",
  xpNone: "@{username} doesn't have a level yet.",
  addcomUsage: "Use: !addcom !trigger reply text",
  addcomSuccess: "Added command {trigger}.",
  addcomExists: "Command {trigger} already exists.",
  delcomUsage: "Use: !delcom !trigger",
  delcomSuccess: "Deleted command {trigger}.",
  delcomMissing: "Command {trigger} not found.",
  editcomUsage: "Use: !editcom !trigger new reply text",
  editcomSuccess: "Updated command {trigger}.",
  editcomMissing: "Command {trigger} not found.",
  commandAdminOnly: "Only admins can manage commands.",
};
...
export const BOT_TRIGGER_FIELDS = [
  ...
  {
    key: "points",
    label: "Points command (balance + admin award)",
  ...
  },
  {
    key: "shoutout",
    label: "Shoutout",
    description: "e.g. !so, !promo",
  },
  {
    key: "uptime",
    label: "Uptime",
    description: "e.g. !uptime",
  },
  {
    key: "followage",
    label: "Followage",
    description: "e.g. !followage",
  },
  {
    key: "watchtime",
    label: "Watchtime",
    description: "e.g. !watchtime",
  },
  {
    key: "level",
    label: "Level",
    description: "e.g. !level",
  },
  {
    key: "xp",
    label: "XP",
  ...
  },
  {
    ...
    description: "Admin only. e.g. !addcom",
  },
  {
    key: "delcom",
    label: "Delete custom command",
    description: "Admin only. e.g. !delcom",
  },
  {
    key: "editcom",
    label: "Edit custom command",
    description: "Admin only. e.g. !editcom",
  },
];
...
function parseTriggerList(raw, fallback) {
  // null/undefined → defaults. Explicit [] or "" → disabled (no triggers).
...
function normalizeTriggers(raw) {
  const next = {};
  for (const key of Object.keys(DEFAULT_BOT_TRIGGERS)) {
    next[key] = parseTriggerList(raw?.[key], DEFAULT_BOT_TRIGGERS[key]);
  }
  return next;
}
...
function normalizeReplies(raw) {
  ...
  for (const field of BOT_REPLY_FIELDS) {
    if (source && Object.prototype.hasOwnProperty.call(source, field.key)) {
      ...
      next[field.key] = DEFAULT_BOT_REPLIES[field.key] || "";
    }
  }
...
function defaultConfig() {
  return {
    ...
    triggers: normalizeTriggers(null),
    customCommands: [],
  };
}
...
export async function botReply(key, vars = {}) {
  ...
  // Empty string is intentional (deleted/silenced reply); only fall back when missing.
  const template =
    Object.prototype.hasOwnProperty.call(replies, key)
      ? replies[key]
      : DEFAULT_BOT_REPLIES[key] || "";
...
export async function addCustomCommand({ trigger, reply }) {
  const config = await getBotConfig();
  const normalizedTrigger = normalizeCommandTrigger(trigger);
  const normalizedReply = String(reply || "").trim().slice(0, 400);
  if (!normalizedTrigger || !normalizedReply) {
    throw new Error("Trigger and reply are required.");
  }
  if (config.customCommands.length >= MAX_CUSTOM_COMMANDS) {
    throw new Error(`Custom command limit reached (${MAX_CUSTOM_COMMANDS}).`);
  }
  if (
    config.customCommands.some((entry) => entry.trigger === normalizedTrigger)
  ) {
    const error = new Error("Command already exists.");
    error.code = "exists";
    error.trigger = normalizedTrigger;
    throw error;
  }
  ...
  await saveBotConfig({ customCommands: next });
  return normalizedTrigger;
}

export async function deleteCustomCommand(trigger) {
  const config = await getBotConfig();
  const normalizedTrigger = normalizeCommandTrigger(trigger);
  const exists = config.customCommands.some(
    (entry) => entry.trigger === normalizedTrigger
  );
  if (!exists) {
  ...
  const next = config.customCommands.filter(
    (entry) => entry.trigger !== normalizedTrigger
  );
  await saveBotConfig({ customCommands: next });
  return normalizedTrigger;
...
export async function editCustomCommand({ trigger, reply }) {
  const config = await getBotConfig();
  const normalizedTrigger = normalizeCommandTrigger(trigger);
  const normalizedReply = String(reply || "").trim().slice(0, 400);
  if (!normalizedTrigger || !normalizedReply) {
  ...
  const index = config.customCommands.findIndex(
    (entry) => entry.trigger === normalizedTrigger
  );
  if (index < 0) {
    const error = new Error("Command not found.");
  ...
  const next = config.customCommands.map((entry, i) =>
    i === index ? { ...entry, reply: normalizedReply, enabled: true } : entry
  );
  await saveBotConfig({ customCommands: next });
  return normalizedTrigger;
}
