...
import { clearGuesses, getClosestGuesses, listGuesses, listGuessRecords, saveGuess } from "./guesses.js";
import {
...
import { getUserByKickId, getAltSoftBlockIndex, withLiveUserBadges } from "./users.js";
import { isActiveKickSubscriber } from "./kick-subscribers.js";

async function buildStakeLinkedPodiumWinners(endingBalance, limit = 3) {
  const records = await listGuessRecords();
  const softBlockIndex = await getAltSoftBlockIndex();
  const linkedGuesses = [];

  for (const guess of records) {
    const kickUserId = String(guess.kickUserId || "").trim();
  ...
  return getClosestGuesses(linkedGuesses, endingBalance, limit);
}
  ...
  try {
    ...
    if (state.endingBalance !== null && state.endingBalance !== undefined) {
      payload.results = {
        ...
        winners: await buildStakeLinkedPodiumWinners(state.endingBalance, 3),
      };
    }
