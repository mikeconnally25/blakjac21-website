...
import { enqueueOverlayAlert } from "./stream-overlay.js";
import {
  botReply,
  getBotConfig,
  matchCommandTrigger,
...
async function handleSubscriptionEvent(rawEvent, eventType) {
  ...
  const alertType =
    String(eventType || "").toLowerCase() === "channel.subscription.renewal"
      ? "resub"
      : "sub";

  try {
    await enqueueOverlayAlert({
      type: alertType,
      username: subscriber.username,
    });
  } catch (error) {
    console.error("Overlay alert enqueue failed:", error.message);
  }
...
async function handleSubscriptionGiftsEvent(rawEvent) {
  ...
  const gifter =
    normalizeKickChatSender(event.gifter) ||
    normalizeKickChatSender(event.sender) ||
    normalizeKickChatSender(event.user);
  const giftCount = Array.isArray(event.giftees)
    ? event.giftees.length
    : saved.length || Number(event.gifter_total) || 1;

  try {
    await enqueueOverlayAlert({
      type: "gift",
      username: gifter?.username || "Someone",
      count: giftCount,
    });
  } catch (error) {
    console.error("Overlay gift alert enqueue failed:", error.message);
  }
