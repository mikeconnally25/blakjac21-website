...
import {
  appendWinnerChatMessage,
  messageMatchesWinner,
} from "./giveaway-winner-chat.js";
import {
  appendKickChatArchiveMessage,
  listRecentChatters,
} from "./kick-chat-archive.js";
import {
  findUserByUsername,
  getUserByKickId,
  withLiveUserBadges,
...
import { awardPoints, awardPointsBulk } from "./points.js";
import {
...
import { maybeSendDueChatPromos } from "./chat-promos.js";
import { creditChatEngagement } from "./chat-engagement.js";
import {
  tryHandleBuiltinChatCommand,
  handlePointsBalanceLookup,
} from "./chat-builtin-commands.js";
import { recordChannelFollow } from "./kick-follows.js";
import { applyLivestreamStatusEvent } from "./stream-status.js";
...
export async function parseSlotRequestCommand(content) {
  const text = cleanChatContent(content);
  const config = await getBotConfig();
  const matched = matchCommandTrigger(text, config.triggers.slot);
  if (!matched) {
    return null;
  }

  return {
    slotQuery: matched.args || "",
  };
}

export async function parsePointsAwardCommand(content) {
  const text = cleanChatContent(content);
  const config = await getBotConfig();

  if (matchCommandTrigger(text, config.triggers.pointsAll)) {
    return null;
  }

  const matched = matchCommandTrigger(text, config.triggers.points);
  if (!matched) {
    return null;
  }

  const argsMatch = matched.args.match(/^(?:@?(\S+))?(?:\s+(-?\d+))?$/i);
  return {
    targetUsername: argsMatch?.[1] || "",
    amount: argsMatch?.[2] !== undefined ? Number(argsMatch[2]) : null,
  };
}

export async function parsePointsAllCommand(content) {
  const text = cleanChatContent(content);
  const config = await getBotConfig();
  const matched = matchCommandTrigger(text, config.triggers.pointsAll);
  if (!matched) {
    return null;
  }

  const argsMatch = matched.args.match(/^(?:(-?\d+))?(?:\s+(\d+))?$/i);
  return {
    amount: argsMatch?.[1] !== undefined ? Number(argsMatch[1]) : null,
    withinMinutes: argsMatch?.[2] !== undefined ? Number(argsMatch[2]) : 15,
  };
}

async function handlePointsAwardCommand({ sender, targetUsername, amount }) {
  const adminName = sender?.username || "admin";
  const adminId = sender?.user_id;

  // No amount → viewer/self balance lookup (works for everyone).
  if (!Number.isFinite(amount)) {
    await handlePointsBalanceLookup({
      sender,
      targetUsername,
      replyToChat,
    });
  ...
  if (
    ...
    await replyToChat(adminName, await botReply("pointsAdminOnly"));
    return;
  }

  if (!targetUsername || amount === 0) {
    await replyToChat(adminName, await botReply("pointsUsage"));
    return;
  }

  try {
    const found = await findUserByUsername(targetUsername);
    if (!found?.kickUserId) {
      await replyToChat(
        adminName,
        await botReply("pointsUserNotFound", {
    ...
    await replyToChat(
      adminName,
      error.message || "Could not update points."
    );
  }
}

async function handlePointsAllCommand({ sender, amount, withinMinutes }) {
  const adminName = sender?.username || "admin";
  const adminId = sender?.user_id;
  ...
  if (!Number.isFinite(amount) || amount === 0) {
    await replyToChat(adminName, await botReply("pointsAllUsage"));
    return;
  }

  try {
    const minutes = Math.max(1, Math.min(120, Number(withinMinutes) || 15));
    const chatters = await listRecentChatters({ withinMinutes: minutes });
    const result = await awardPointsBulk({
      users: chatters,
      amount,
      note: `Chat bulk award (${minutes}m)`,
      actorKickUserId: adminId,
      actorUsername: adminName,
    });
    ...
    await replyToChat(
      adminName,
      await botReply("pointsAllAwarded", {
        amount: signedAmount,
        count: result.awarded,
        minutes,
      })
    );
  } catch (error) {
    await replyToChat(
      adminName,
      error.message || "Could not award points to chat."
    );
  }
}
...
async function handleSlotRequestCommand({ sender, slotQuery }) {
  ...
  if (!slotQuery) {
    await replyToChat(username, await botReply("slotUsage"));
    return;
  }

  if (!(await areSlotRequestsOpen())) {
    await replyToChat(username, await botReply("slotClosed"));
    return;
  }
  ...
  try {
    ...
    if (request.groupSlug === "pending") {
      await replyToChat(
        username,
        await botReply("slotSuccessPending", { slot: request.slotName })
      );
    } else {
      await replyToChat(
        username,
        await botReply("slotSuccess", {
          slot: request.slotName,
          group: request.groupLabel,
        })
      );
    }
  } catch (error) {
...
async function captureWinnerChatMessage({ sender, content }) {
  try {
    ...
    return await appendWinnerChatMessage({
      kickUserId,
      username,
      text,
    });
  } catch (error) {
    console.error("Winner chat capture failed:", error.message);
    return null;
  }
}
...
async function handleGiveawayKeywordEntry({ sender, content }) {
  ...
  if (state.affiliatesOnly || state.subscribersOnly) {
    ...
    if (!allowed) {
      let message = await botReply("giveawayAffSubOnly");
      if (state.affiliatesOnly && !state.subscribersOnly) {
        message = await botReply("giveawayAffOnly");
      } else if (state.subscribersOnly && !state.affiliatesOnly) {
        message = await botReply("giveawaySubOnly");
      }
  ...
  try {
    ...
    if (result.alreadyEntered) {
      await replyToChat(username, await botReply("giveawayAlreadyEntered"));
      return {
        handled: true,
    ...
    await replyToChat(username, await botReply("giveawayEntered"));
    return {
      handled: true,
...
async function archiveChatMessage({ sender, content }) {
  try {
    const kickUserId = sender?.user_id;
    const username = sender?.username || "viewer";
    const text = cleanChatContent(content);
    ...
    return await appendKickChatArchiveMessage({
      kickUserId,
      username,
      text,
    });
  } catch (error) {
    console.error("Kick chat archive failed:", error.message);
    return null;
  }
}
...
async function handleChatMessage(rawEvent) {
  ...
  const archived = await archiveChatMessage({ sender, content });

  creditChatEngagement({
    kickUserId: sender?.user_id,
    username: sender?.username,
  }).catch((error) => {
    console.error("Chat engagement credit failed:", error.message);
  });

  const pointsAllCommand = await parsePointsAllCommand(content);
  if (pointsAllCommand) {
    await handlePointsAllCommand({
      sender,
      amount: pointsAllCommand.amount,
      withinMinutes: pointsAllCommand.withinMinutes,
    });
    return {
      handled: true,
      reason: "points-award-all",
  ...
  const pointsCommand = await parsePointsAwardCommand(content);
  if (pointsCommand) {
    await handlePointsAwardCommand({
      sender,
      targetUsername: pointsCommand.targetUsername,
      amount: pointsCommand.amount,
    });
    return {
      handled: true,
      reason: Number.isFinite(pointsCommand.amount)
        ? "points-award"
        : "points-balance",
      username: sender?.username || null,
      trackedSubscriber,
  ...
  const builtin = await tryHandleBuiltinChatCommand({
    sender,
    content: cleanChatContent(content),
    replyToChat,
  });
  if (builtin.handled) {
    return {
  ...
  const command = await parseSlotRequestCommand(content);
  ...
  const customCommand = await matchCustomCommand(content);
  if (customCommand) {
    const username = sender?.username || "viewer";
    await replyToChat(
      username,
      formatBotReply(customCommand.reply, {
        username,
  ...
  const winnerChatResult = await captureWinnerChatMessage({ sender, content });

  // Opportunistic promo check when chat is active (cron is the primary scheduler).
  maybeSendDueChatPromos().catch((error) => {
    console.error("Chat promo check failed:", error.message);
  });

  return {
    ...giveawayResult,
    winnerChatCaptured: Boolean(winnerChatResult),
    trackedSubscriber,
    archived: Boolean(archived),
  };
}
...
async function handleSubscriptionEvent(rawEvent, eventType) {
...
async function handleChannelFollowedEvent(rawEvent) {
  const event = normalizeKickWebhookEvent(rawEvent);
  if (!event) {
    return { handled: false, reason: "unrecognized-event" };
  }
  ...
  const follower = normalizeKickChatSender(event.follower || event.user);
  if (!follower) {
    return { handled: false, reason: "missing-follower" };
  }

  const saved = await recordChannelFollow({
    kickUserId: follower.user_id,
  ...
  return {
    handled: true,
    reason: "follow-recorded",
    kickUserId: String(follower.user_id),
    followedAt: saved?.followedAt || null,
  };
}

async function handleLivestreamStatusEvent(rawEvent) {
  const status = await applyLivestreamStatusEvent(rawEvent);
  return {
    handled: true,
    reason: "livestream-status",
...
async function routeKickWebhookEvent(eventType, rawEvent) {
  ...
  if (
    ...
    return handleSubscriptionEvent(rawEvent, normalizedType);
  }
  ...
  if (normalizedType === "channel.followed") {
    return handleChannelFollowedEvent(rawEvent);
  }

  if (normalizedType === "livestream.status.updated") {
    return handleLivestreamStatusEvent(rawEvent);
  }
