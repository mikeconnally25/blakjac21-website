...
import { addSlotTournamentEntry } from "./slot-tournament-entries.js";
import { getSlotTournamentState } from "./slot-tournament-state.js";
import {
...
export function parsePointsAwardCommand(content) {
  const text = cleanChatContent(content);
  if (/^!pointsall\b/i.test(text)) {
    return null;
  }

  const match = text.match(/^!points(?:\s+@?(\S+))?(?:\s+(-?\d+))?$/i);
  if (!match) {
    return null;
  }
  ...
  return {
    ...
    amount: match[2] !== undefined ? Number(match[2]) : null,
  };
}

export function parsePointsAllCommand(content) {
  const text = cleanChatContent(content);
  const match = text.match(/^!pointsall(?:\s+(-?\d+))?(?:\s+(\d+))?$/i);
  if (!match) {
    return null;
  }

  return {
    amount: match[1] !== undefined ? Number(match[1]) : null,
    withinMinutes: match[2] !== undefined ? Number(match[2]) : 15,
  };
}
...
async function handlePointsAwardCommand({ sender, targetUsername, amount }) {
  ...
  if (
    ...
    await replyToChat(adminName, "Only admins can award points.");
    return;
  }

  if (!targetUsername || !Number.isFinite(amount) || amount === 0) {
    await replyToChat(adminName, "Use: !points @username 100");
    return;
  }
  ...
  try {
    ...
    if (!found?.kickUserId) {
      await replyToChat(
        adminName,
        `Could not find @${targetUsername.replace(/^@/, "")} — they need to sign in on the site first.`
      );
      return;
    }
    ...
    await replyToChat(
      adminName,
      `${verb} ${amount > 0 ? "+" : ""}${amount} points to @${found.username}. Balance: ${balance.points}.`
    );
  } catch (error) {
    await replyToChat(
      adminName,
...
async function handlePointsAllCommand({ sender, amount, withinMinutes }) {
  ...
  try {
    ...
    await replyToChat(
      adminName,
      `Awarded ${amount > 0 ? "+" : ""}${amount} points to ${result.awarded} chatters active in the last ${minutes}m.`
    );
  } catch (error) {
    await replyToChat(
      adminName,
...
async function handleSlotTournamentKeywordEntry({ sender, content }) {
  const username = sender?.username || "viewer";
  const kickUserId = sender?.user_id;
  ...
  if (!state.open || !state.keyword) {
    return { handled: false, reason: "tournament-closed" };
  }
  ...
  const keyword = String(state.keyword || "")
    ...
    .trim()
    .toLowerCase();
  const isJoinCommand = /^!(?:tour|tournament|st)\b/i.test(cleaned);
  const isKeyword = Boolean(cleaned && keyword && cleaned === keyword);

  if (!isJoinCommand && !isKeyword) {
    return { handled: false, reason: "tournament-keyword-mismatch" };
  }
  ...
  if (state.affiliatesOnly || state.subscribersOnly) {
    ...
    if (!allowed) {
      let message =
        "This tournament is restricted. Verify Stake with BLAKJAC21 or be an active Kick sub.";
      if (state.affiliatesOnly && !state.subscribersOnly) {
        message =
      ...
      return {
        handled: true,
        reason: "tournament-access-restricted",
        username,
      };
    }
  }
  ...
  try {
    const result = await addSlotTournamentEntry({ kickUserId, username });

    if (result.alreadyEntered) {
      await replyToChat(username, "You're already signed up for the tournament.");
      return {
        handled: true,
        reason: "tournament-already-entered",
        username,
      };
    }

    await replyToChat(username, "You're signed up for the slot tournament!");
    return {
      handled: true,
      reason: "tournament-entered",
      username,
    };
  } catch (error) {
    await replyToChat(username, error.message);
...
async function handleChatMessage(rawEvent) {
  ...
  const pointsAllCommand = parsePointsAllCommand(content);
  if (pointsAllCommand) {
  ...
  const pointsCommand = parsePointsAwardCommand(content);
  if (pointsCommand) {
  ...
  const tournamentResult = await handleSlotTournamentKeywordEntry({
    sender,
    content,
  });
  if (tournamentResult.handled) {
    return {
      ...tournamentResult,
      trackedSubscriber,
      archived: Boolean(archived),
