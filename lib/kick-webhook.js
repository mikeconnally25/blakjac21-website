...
async function readRawBody(req) {
  if (Buffer.isBuffer(req.body)) {
    return { rawBody: req.body.toString("utf8"), preParsed: false };
  }

  if (typeof req.body === "string") {
    return { rawBody: req.body, preParsed: false };
  }
  ...
  if (streamed) {
    return { rawBody: streamed, preParsed: false };
  }

  if (req.body !== undefined && req.body !== null && typeof req.body === "object") {
    return { rawBody: JSON.stringify(req.body), preParsed: true };
  }
...
async function handleChatMessage(rawEvent) {
  ...
  if (!command) {
    return { handled: false, reason: "not-command", content };
  }
