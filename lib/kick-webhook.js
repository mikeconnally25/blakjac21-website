...
async function handleGiveawayKeywordEntry({ sender, content }) {
  ...
  if (state.affiliatesOnly) {
    const storedUser = await getUserByKickId(kickUserId);
    const verifiedUser = storedUser
      ? await withLiveStakeCodeVerification(storedUser)
      : null;

    if (!verifiedUser?.stakeCodeVerified) {
      await replyToChat(
        username,
      ...
      return {
        handled: true,
        reason: "giveaway-affiliates-only",
        username,
      };
    }
  }
