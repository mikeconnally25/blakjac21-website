...
export function parseSlotRequestCommand(content) {
  ...
  if (!remainder) {
    return { slotQuery: "", bet: null };
  }

  const betMatch = remainder.match(/^(.*?)(?:\s+)(\$?\d+(?:\.\d{1,2})?)$/);
  if (betMatch) {
    const bet = Number(betMatch[2].replace(/^\$/, ""));
    if (Number.isFinite(bet) && bet > 0) {
      return {
        slotQuery: betMatch[1].trim(),
...
async function handleSlotRequestCommand({ sender, slotQuery, bet }) {
  ...
  if (!slotQuery) {
    await replyToChat(
      username,
      "Use: !s <slot name> <bet> — New Releases or Only on Stake slots only."
    );
    return;
  }
  ...
  if (bet === null || bet === undefined) {
    await replyToChat(
      username,
      "Include a bet size. Example: !s Gates of Olympus 2"
    );
    return;
  }
  ...
  try {
    ...
    await replyToChat(
      username,
      `Requested ${request.slotName} at $${request.bet.toFixed(2)} (${request.groupLabel}).`
    );
  } catch (error) {
