...
export async function findPointsBalanceByUsername(username) {
  ...
  const store = await readStore();
  for (const entry of Object.values(store.balances)) {
    if (String(entry?.username || "").toLowerCase() === needle) {
      return normalizeBalance(entry, entry.kickUserId);
    }
  }
  return null;
}
...
/**
 * Deduct points for a wager. Fails if balance is too low.
 */
export async function spendPoints({
  kickUserId,
  username,
  amount,
  note = "House game wager",
}) {
  const userId = String(kickUserId || "").trim();
  ...
  if (existing.points < cost) {
    throw new Error(
      `Not enough points. You have ${existing.points}; this bet needs ${cost}.`
    );
  }

  const now = new Date().toISOString();
  const nextPoints = existing.points - cost;
  const displayName =
...
/**
 * Move points from one signed-in user to another in a single store write.
 */
export async function tipPoints({
  ...
  const fromId = String(fromKickUserId || "").trim();
  const toId = String(toKickUserId || "").trim();
  const tipAmount = Math.floor(Number(amount));

  if (!fromId) {
    throw new Error("Sign in to tip.");
  }
  if (!toId) {
    throw new Error("Choose a user to tip.");
  }
  if (fromId === toId) {
    throw new Error("You cannot tip yourself.");
  }
  if (!Number.isFinite(tipAmount) || tipAmount < 1) {
    throw new Error("Tip at least 1 point.");
  }

  const store = await readStore();
  const fromExisting = normalizeBalance(store.balances[fromId], fromId);
  if (fromExisting.points < tipAmount) {
    throw new Error(
      `Not enough points. You have ${fromExisting.points}; this tip needs ${tipAmount}.`
    );
  }

  const toExisting = normalizeBalance(store.balances[toId], toId);
  const now = new Date().toISOString();
  const fromName =
    String(fromUsername || fromExisting.username || "viewer").trim() || "viewer";
  const toName =
    String(toUsername || toExisting.username || "viewer").trim() || "viewer";

  const fromNext = fromExisting.points - tipAmount;
  const toNext = toExisting.points + tipAmount;

  store.balances[fromId] = {
    kickUserId: fromId,
    points: fromNext,
    username: fromName,
    updatedAt: now,
  ...
  await writeStore(store);
  return {
    from: store.balances[fromId],
    to: store.balances[toId],
    amount: tipAmount,
  };
}

/**
 * Credit winnings (or refund on push). amount is points added.
 */
export async function creditPoints({
  kickUserId,
  username,
  amount,
  note = "House game payout",
}) {
  const userId = String(kickUserId || "").trim();
...
export async function deleteRedemption(id) {
  ...
  const current = store.redemptions[index];
  if (current.status === "pending") {
    throw new Error("Cancel pending redemptions instead of deleting them.");
  }

  const [removed] = store.redemptions.splice(index, 1);
  await writeStore(store);
  return removed;
}
