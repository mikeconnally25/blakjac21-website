...
import {
  ...
  getPurchaseById,
  getPurchaseLimits,
  getPointsBuySettings,
  isPointsBuyingEnabled,
  listPointPackages,
  markPurchasePaid,
  resolvePurchaseAmount,
  revertPurchasePaid,
  setPointsBuyingEnabled,
...
export async function handlePointsBuySettings(req, res) {
  if (req.method !== "POST") {
    sendJson(res, 405, { error: "Method not allowed" });
    return;
  }
  ...
  try {
    const settings = await setPointsBuyingEnabled(body.enabled);
    sendJson(res, 200, {
      ok: true,
    ...
    sendJson(res, 500, {
      error: error.message || "Could not update buy points settings.",
    });
  }
}
...
export async function handlePointsBuy(req, res) {
  ...
  if (!(await isPointsBuyingEnabled())) {
    sendJson(res, 503, {
      error: "Buying points is paused right now. Try again later.",
    });
    return;
  }
