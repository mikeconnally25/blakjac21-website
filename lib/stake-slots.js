...
function getGraphqlOrigin(graphqlUrl) {
  return graphqlUrl.includes("stake.bet")
    ? "https://stake.bet"
    : "https://stake.com";
}

function buildGraphqlHeaders(group, graphqlUrl) {
  const origin = getGraphqlOrigin(graphqlUrl);
  const headers = {
    ...
    Origin: origin,
    Referer: `${origin}/casino/group/${group.slug}`,
  };
...
export function formatStakeCatalogError(error) {
  ...
  if (/403/.test(message)) {
    if (!getStakeAccessToken()) {
      return `${message} Stake blocks server requests without a token. Create one at stake.com → Settings → Security → API Tokens, add STAKE_ACCESS_TOKEN in Vercel, redeploy, then click Refresh slot list. You can also run npm run refresh:slots locally and commit catalog/stake-allowed-slots.json.`;
    }

    return `${message} Check that STAKE_ACCESS_TOKEN in Vercel is valid and not expired.`;
  }

  if (!getStakeAccessToken()) {
    return `${message} Add STAKE_ACCESS_TOKEN in Vercel, or run npm run refresh:slots locally and commit catalog/stake-allowed-slots.json.`;
  }
...
async function fetchGroupSlotsFromGraphql(group, graphqlUrl) {
  ...
  let offset = 0;
  let pages = 0;
  const headers = buildGraphqlHeaders(group, graphqlUrl);
