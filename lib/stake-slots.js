...
export function formatStakeCatalogError(error) {
  ...
  if (/403/.test(message)) {
    return `${message} Stake blocked the request (Cloudflare). Add or refresh STAKE_ACCESS_TOKEN and STAKE_CF_CLEARANCE in Vercel, then redeploy.`;
  }

  if (!getStakeAccessToken()) {
    return `${message} Add STAKE_ACCESS_TOKEN in Vercel (and STAKE_CF_CLEARANCE if Cloudflare still blocks), then redeploy.`;
  }
