...
export async function listGuessRecords() {
  const store = await readStore();
  return sortGuesses(store.guesses);
}

export async function listGuessUserProfiles() {
  const store = await readStore();
  const profiles = new Map();

  for (const guess of store.guesses) {
    const kickUserId = String(guess.kickUserId || "").trim();
    if (!kickUserId) continue;

    const seenAt = guess.submittedAt || new Date().toISOString();
    ...
    if (new Date(seenAt) > new Date(existing.lastSeenAt)) {
      ...
      existing.username = guess.username || existing.username;
    }
...
export function getClosestGuesses(guesses, endingBalance, limit = 3) {
  ...
  return [...guesses]
    .map((guess) => ({
      ...guess,
      difference: Math.abs(Number(guess.amount) - target),
    }))
    .sort((a, b) => {
    ...
    .map((entry, index) => ({
      ...
      stakeUsername: String(entry.stakeUsername || "").trim() || null,
      amount: entry.amount,
