...
import { isAltImmune } from "./alt-immunity.js";
...
function normalizeLoginHistory(history) {
  ...
  return history
    ...
    .map((entry) => {
      const rawIp = normalizeIp(entry.ip);
      return {
        ip: rawIp && !isIgnoredLoginIp(rawIp) ? rawIp : null,
        at: entry.at || null,
...
function appendLoginHistory(existingHistory, loginMeta = null) {
  ...
  if (!loginMeta || typeof loginMeta !== "object") {
    return history;
  }

  const rawIp = normalizeIp(loginMeta.ip);
  // Keep a login event even when IP is missing/local; only store public IPs.
  const ip = rawIp && !isIgnoredLoginIp(rawIp) ? rawIp : null;
...
function mergeActivityProfile(store, profile) {
  ...
  const unchanged =
    ...
    (existing.banReason || null) === (stored.banReason || null) &&
...
export async function upsertKickUser(kickProfile, loginMeta = null) {
  ...
  const user = await ensureUserRecord({
    ...
    profilePicture: kickProfile.profile_picture || null,
    loginMeta: {
      ...(loginMeta && typeof loginMeta === "object" ? loginMeta : {}),
      at: loginMeta?.at || new Date().toISOString(),
    },
  });

  try {
    await enforceAltBans();
  } catch (error) {
    console.error("Could not enforce alt bans after login:", error.message);
  }

  const refreshed = (await getUserByKickId(kickUserId)) || user;
  return {
    ...refreshed,
    isNew: user.isNew,
  };
...
/** One users-store read for many Kick IDs (avoids N Redis GETs). */
export async function getUsersByKickIds(kickUserIds) {
  const ids = [
    ...new Set(
      (Array.isArray(kickUserIds) ? kickUserIds : [])
  ...
  try {
    const store = await readStore();
    for (const id of ids) {
      byId.set(id, store.users[id] || null);
    }
  } catch {
    for (const id of ids) {
      byId.set(id, null);
...
function userLooksLikeAdmin(user) {
  return isKickAdmin({
    name: user?.username,
    username: user?.username,
    user_id: user?.kickUserId,
    kickUserId: user?.kickUserId,
  });
}

function userHasAltImmunity(user) {
  return isAltImmune({
    name: user?.username,
    username: user?.username,
    user_id: user?.kickUserId,
...
export async function setUserBanned(kickUserId, banned, banReason = null) {
  const userId = String(kickUserId || "").trim();
  if (!userId) {
    throw new Error("User id is required.");
  }
  ...
  if (nextBanned && userLooksLikeAdmin(existing)) {
  ...
  const updated = toStoredUser({
    ...
    banReason: nextBanned ? banReason || existing.banReason || "manual" : null,
    loginHistory: normalizeLoginHistory(existing.loginHistory),
  });
...
/**
 * Permanently ban every non-primary account in an IP-based alt cluster.
 * Oldest account stays; newer accounts on the same or ≥70%-similar IPs are banned.
 * Admins and alt-immune accounts are never banned (and any existing ban is cleared).
 */
export async function enforceAltBans() {
  const store = await readStore();
  ...
  for (const user of users) {
    if (userLooksLikeAdmin(user) || userHasAltImmunity(user)) {
      if (user.banned) {
        store.users[user.kickUserId] = toStoredUser({
          ...user,
          banned: false,
          bannedAt: null,
          banReason: null,
    ...
    const soft = softBlockIndex.get(user.kickUserId);
    if (!soft?.softBlocked) continue;
    ...
    changed = true;
    bannedIds.push(user.kickUserId);
  }
...
export async function removeUser(kickUserId) {
  ...
  if (store.users[userId].banned) {
    throw new Error(
      "Banned accounts cannot be removed. Keeping them blocked stops re-registration."
    );
  }
  ...
  return [
    ...new Set(
      normalizeLoginHistory(user?.loginHistory)
        .map((entry) => entry.ip)
        .filter((ip) => ip && !isIgnoredLoginIp(ip))
    ),
  ];
}
...
/** Minimum string similarity (0–1) for two login IPs to count as the same alt signal. */
const ALT_IP_SIMILARITY_THRESHOLD = 0.7;
...
export function ipSimilarity(a, b) {
...
function buildPossibleAlts(users) {
  ...
  const ips = [...ipOwners.keys()];
  const parent = new Map(ips.map((ip) => [ip, ip]));
  ...
  for (let i = 0; i < ips.length; i += 1) {
    for (let j = i + 1; j < ips.length; j += 1) {
      if (ipSimilarity(ips[i], ips[j]) >= ALT_IP_SIMILARITY_THRESHOLD) {
        union(ips[i], ips[j]);
      }
    }
  }
  ...
  for (const ip of ips) {
    const root = find(ip);
    if (!ipGroups.has(root)) {
      ipGroups.set(root, []);
    }
    ipGroups.get(root).push(ip);
  }

  const byUser = new Map();

  for (const groupIps of ipGroups.values()) {
    const ownersById = new Map();
    const ownerIps = new Map();

    for (const ip of groupIps) {
      for (const user of ipOwners.get(ip) || []) {
        ownersById.set(user.kickUserId, user);
        if (!ownerIps.has(user.kickUserId)) {
          ownerIps.set(user.kickUserId, new Set());
        }
        ownerIps.get(user.kickUserId).add(ip);
      }
    }

    if (ownersById.size < 2) {
      continue;
    }
    ...
    for (const user of owners) {
      ...
      const userIps = ownerIps.get(user.kickUserId) || new Set();
      ...
      for (const other of owners) {
        ...
        const otherIps = ownerIps.get(other.kickUserId) || new Set();
        const relatedIps = [...new Set([...userIps, ...otherIps])].sort();
...
export function buildAltSoftBlockIndex(users) {
  ...
  for (const cluster of clusters) {
    ...
    for (const member of cluster.users) {
      const immune = userHasAltImmunity(member);
      index.set(member.kickUserId, {
        softBlocked: !immune && member.kickUserId !== primaryKickUserId,
        primaryKickUserId,
...
const SOFT_BLOCK_CACHE_TTL_MS = 30_000;
let softBlockCache = { at: 0, index: null };

async function getCachedAltSoftBlockIndex() {
  if (
    softBlockCache.index &&
    Date.now() - softBlockCache.at < SOFT_BLOCK_CACHE_TTL_MS
  ) {
    return softBlockCache.index;
  }
  const index = await getAltSoftBlockIndex();
  softBlockCache = { at: Date.now(), index };
  return index;
}

/**
 * Banned or similar-IP soft-blocked accounts cannot participate on the site or via Kick chat.
 * @returns {{ blocked: boolean, reason: string|null }}
 */
export async function getParticipationBlock(kickUserId) {
  const id = String(kickUserId || "").trim();
  if (!id) {
    return { blocked: false, reason: null };
  }

  const user = await getUserByKickId(id);
  if (user?.banned) {
    return {
      blocked: true,
      reason: String(user.banReason || "banned"),
    };
  }
  ...
  try {
    const soft = (await getCachedAltSoftBlockIndex()).get(id);
    if (soft?.softBlocked) {
      return { blocked: true, reason: "alt" };
    }
  } catch (error) {
    console.error("Participation soft-block check failed:", error.message);
  }

  return { blocked: false, reason: null };
}

export async function isParticipationBlocked(kickUserId) {
  const result = await getParticipationBlock(kickUserId);
  return result.blocked;
}

export async function assertCanParticipate(kickUserId) {
  const result = await getParticipationBlock(kickUserId);
  if (!result.blocked) return;
  if (result.reason === "alt") {
    throw new Error("This account is blocked as an alt and cannot participate.");
  }
  throw new Error("This account is banned and cannot participate.");
}
...
export async function listUsers() {
  ...
  try {
    ...
    console.error("Could not enforce alt bans:", error.message);
  }
  const store = await readStore();
  ...
  const withBadges = users.map((user) => {
    ...
    return {
      ...user,
      affGranted,
      isAdmin: userLooksLikeAdmin(user),
      altImmune: userHasAltImmunity(user),
      banned: Boolean(user.banned),
      bannedAt: user.banned ? user.bannedAt || null : null,
      banReason: user.banned ? user.banReason || null : null,
      affOnRoster,
      stakeCodeVerified,
      stakeCodeVerifiedAt: stakeCodeVerified
        ? user.stakeCodeVerifiedAt || new Date().toISOString()
