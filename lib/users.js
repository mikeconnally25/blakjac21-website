...
export async function linkStakeUsername(kickUserId, rawStakeUsername) {
  const userId = String(kickUserId || "").trim();
  ...
  const stakeCodeVerified = await isStakeUsernameOnAffiliateCode(stakeUsername);
...
export async function listUsers() {
  ...
  return Promise.all(users.map((user) => withLiveStakeCodeVerification(user)));
}
