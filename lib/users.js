...
async function writeRedisStore(store) {
  ...
  // Prefer body-based SET so profile picture URLs don't blow URL length limits.
  // Upstash expects the body to be a JSON-encoded string value.
  const bodyResponse = await fetch(`${config.url}/set/${USERS_KEY}`, {
    method: "POST",
    headers,
  ...
  if (bodyResponse.ok) {
    const data = await bodyResponse.json().catch(() => ({}));
    if (data.result === "OK") {
      return true;
    }
  }

  // Fallback to path-encoded SET used by other stores.
  const payload = encodeURIComponent(value);
  const pathResponse = await fetch(`${config.url}/set/${USERS_KEY}/${payload}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${config.token}` },
    cache: "no-store",
  });

  if (!pathResponse.ok) return false;

  const pathData = await pathResponse.json().catch(() => ({}));
  return pathData.result === "OK";
}
