...
import { canSelfCreditHousePoints } from "./house-games.js";
...
export async function handlePointsMe(req, res) {
  ...
  try {
    ...
    sendJson(res, 200, {
      ...
      redemptions,
      canRedeem: !canSelfCreditHousePoints(session.username),
      canTip: !canSelfCreditHousePoints(session.username),
    });
  } catch {
...
export async function handlePointsTip(req, res) {
  const session = await getSession(req);
  if (!session?.kickUserId) {
    sendJson(res, 401, { error: "Sign in with Kick first." });
    return;
  ...
  if (canSelfCreditHousePoints(session.username)) {
    sendJson(res, 403, {
      error: "Uncapped house-game accounts cannot tip points.",
    });
    return;
  }
  ...
  try {
    const username = String(body.username || "")
      ...
      .trim();
    if (!username) {
      sendJson(res, 400, { error: "Enter a Kick username to tip." });
      return;
    }
    ...
    if (signedIn?.kickUserId) {
      toKickUserId = signedIn.kickUserId;
      toUsername = signedIn.username || username;
    } else {
      const balanceHit = await findPointsBalanceByUsername(username);
      if (balanceHit?.kickUserId) {
        toKickUserId = balanceHit.kickUserId;
        toUsername = balanceHit.username || username;
      }
    }

    if (!toKickUserId) {
      sendJson(res, 404, {
        error: "User not found. They need to have signed in on the site first.",
      });
      return;
    }

    if (String(toKickUserId) === String(session.kickUserId)) {
      sendJson(res, 400, { error: "You cannot tip yourself." });
      return;
    }

    const result = await tipPoints({
      fromKickUserId: session.kickUserId,
    ...
    sendJson(res, 200, {
      ...
      amount: result.amount,
      to: {
        kickUserId: result.to.kickUserId,
        username: result.to.username,
        points: result.to.points,
      },
      balance: result.from,
    });
  } catch (error) {
    sendJson(res, 400, { error: error.message || "Could not send tip." });
  }
}
...
export async function handlePointsRedeem(req, res) {
  ...
  if (canSelfCreditHousePoints(session.username)) {
    sendJson(res, 403, {
      error: "This account cannot redeem store rewards.",
    });
    return;
  }
